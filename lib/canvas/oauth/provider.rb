...
module Canvas::Oauth
  class Provider
    ...
    def self.confirmation_redirect(controller, provider, logged_in_user)
      # skip the confirmation page if access is already (or automatically) granted
      if provider.authorized_token?(logged_in_user)
        final_redirect(controller, final_redirect_params(controller.session[:oauth2], logged_in_user))
      else
        controller.oauth2_auth_confirm_url
      end
    end

    def self.final_redirect_params(oauth_session, logged_in_user, options = {})
      options = {:scopes => oauth_session[:scopes], :remember_access => options[:remember_access], :purpose => oauth_session[:purpose]}
      code = Canvas::Oauth::Token.generate_code_for(logged_in_user.global_id, oauth_session[:client_id], options)
