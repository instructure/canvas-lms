...
module SIS
  module CSV
    class EnrollmentImporter < BaseImporter
      ...
      def process(csv)
        messages = []
        @sis.counts[:enrollments] += SIS::EnrollmentImporter.new(@batch.try(:id), @root_account, logger, @override_sis_stickiness).process(messages, @sis.updates_every) do |importer|
          csv_rows(csv) do |row|
            update_progress
