...
module DataFixup::Auditors::MigrateCoursesToPartitions
  def self.run(batch_size: 1000, last_run_date_threshold: nil)
    partman = CanvasPartman::PartitionManager.create(::Auditors::ActiveRecord::CourseRecord)
    if partman.migrate_data_to_partitions(timeout: 5.minutes, batch_size: batch_size)
      GuardRail.activate(:deploy) { Message.connection.update("TRUNCATE ONLY #{::Auditors::ActiveRecord::CourseRecord.quoted_table_name}") }
    else
