...
var wikiSidebar = {
  ...
  itemSelected: function(item) {
    switch (item.item_type) {
      case 'image':
        wikiSidebar.editor.editorBox(
          'insert_code',
          '<img alt="' + htmlEscape(item.title) + '" src="' + htmlEscape(item.link_url) + '"/>'
        )
        break
      default:
        // we'll rely on enhance-user-content to create youtube thumbnails, etc.
        wikiSidebar.editor.editorBox('create_link', {
          title: item.title || I18n.t('no_title', 'No title'),
          url: item.link_url
        })
        break
    }
  ...
  fileSelected: function(node) {
    ...
    wikiSidebar.editor.editorBox('create_link', {
      ...
      url: url,
      file: true,
      image: node.hasClass('image'),
      scribdable: node.hasClass('scribdable'),
      kaltura_entry_id: node.attr('data-media-entry-id'),
      kaltura_media_type: node.hasClass('video_playback') ? 'video' : 'audio'
    })
  },
  imageSelected: function($img) {
    ...
    wikiSidebar.editor.editorBox(
      'insert_code',
      '<img alt="' + htmlEscape(alt) + '" src="' + htmlEscape(src) + '"/>'
    )
  },
  fileAdded: function(attachment, newUploadOrCallbackOrParent, fileCallback) {
    ...
    if (children.length || fileCallback) {
      ...
      var displayName =
        ...
        htmlEscape(file.display_name) +
        ' ' +
        htmlEscape(I18n.t('aria_tree.file', 'file')) +
        '</span>' +
        htmlEscape(file.display_name)
      ...
      if (file.media_entry_id) {
        $file
          ...
          .addClass(
            file.content_type && file.content_type.match(/video/)
              ? 'video_playback'
              : 'audio_playback'
          )
      }
    ...
    if (
      newUpload &&
      (attachment.mime_class == 'image' || attachment.content_type.match(/^image/)) &&
      ...
      function(data) {
        ...
        for (var idx in data.sub_folders) {
          ...
          $folder
            .attr('class', 'folder')
            .data('id', folder.id)
            .addClass('folder_' + folder.id)
          $folder
            .find('.name')
            .html(
              " <span class='screenreader-only'>" +
                htmlEscape(folder.name) +
                ' ' +
                htmlEscape(I18n.t('aria_tree.folder', 'folder')) +
                '</span>' +
                htmlEscape(folder.name)
            )
          $folder
  ...
  init: function() {
      ...
      $sidebar_upload_image_form.slideToggle('fast', function() {
        ...
        if ($imageForm.is(':visible')) {
          ...
          $(event.currentTarget).attr(
            'aria-label',
            I18n.t('image_form.expanded', 'Click to toggle the new image form (expanded)')
          )
        } else {
          $(event.currentTarget).attr(
            'aria-label',
            I18n.t('image_form.collapsed', 'Click to toggle the new image form (collapsed)')
          )
        }
      })
    })
    ...
    if ($.handlesHTML5Files) {
      $('#editor_tabs_3 .file_list_holder')
        .bind(
          'dragenter dragover',
          function(event) {
            if (!$(this).hasClass('file_drop')) {
              return
            }
            event.preventDefault()
        ...
        .bind(
          ...
          function(event) {
            ...
            var folderSelect = function(folder_id) {
              $('#wiki_sidebar_file_uploads').triggerHandler('files_added', {
                files: filesToUpload,
                folder_id: folder_id
              })
            }
      ...
      $('#editor_tabs_4 .image_list_holder')
        .bind(
          'dragenter dragover',
          function(event) {
            if (!$(this).hasClass('file_drop')) {
        ...
        .bind(
          ...
          function(event) {
            ...
            if (images.length === 0) {
              alert(
                I18n.t('errors.no_valid_image_files_selected', 'No valid image files were selected')
              )
              return
            }
            var folderSelect = function(folder_id) {
              $('#wiki_sidebar_image_uploads').triggerHandler('files_added', {
                files: images,
                folder_id: folder_id
              })
            }
    ...
    $('#wiki_sidebar_file_uploads')
      .bind('files_added', function(event, data) {
        for (var idx in data.files) {
      ...
      .bind('file_upload_check', function(event) {
        ...
        if (fileUploadsReady) {
          ...
          $list.fillTemplateData({
            data: {filename: fileData.name, files_remaining: fileUploadsFileList.length}
          })
          ...
          $.ajaxFileUpload({
            ...
            error: function(data) {
              $.flashError(
                I18n.t(
                  'errors.unexpected_upload_problem',
    ...
    $('#wiki_sidebar_image_uploads')
      ...
      .bind('file_upload_check', function(event) {
        ...
        if (imageUploadsReady) {
          ...
          var fileData = $.fileData(fileWrapper.file)
          $list.fillTemplateData({
            data: {filename: fileData.name, files_remaining: imageUploadsFileList.length}
          })
    ...
    $sidebar_upload_image_form.formSubmit({
      ...
      folder_id: function() {
        return $(this)
          .find("[name='attachment[folder_id]']")
          .val()
      },
      upload_only: true,
      object_name: 'attachment',
      processData: function(data) {
        data['attachment[display_name]'] = $(this)
          .find('.file_name')
          .val()
        return data
      },
      beforeSubmit: function(data) {
        ...
        $sidebar_upload_image_form.attr(
          'action',
          $sidebar_upload_image_form.find('.json_upload_url').attr('href')
        )
      },
      success: function(data) {
    ...
    $sidebar_upload_file_form.formSubmit({
      ...
      beforeSubmit: function(data) {
        ...
        $sidebar_upload_file_form.attr(
          'action',
          $sidebar_upload_file_form.find('.json_upload_url').attr('href')
        )
        $(this)
          .find('button')
          .attr('disabled', true)
          .text(I18n.t('buttons.uploading', 'Uploading...'))
      },
      success: function(data) {
        $(this)
          .find('button')
      ...
      error: function(data) {
        $(this)
          .find('button')
          .attr('disabled', false)
          .text(I18n.t('errors.upload_failed', 'Upload Failed, please try again'))
