...
window.modules = (function() {
  return {
    ...
    addModule: function() {
      var $module = $('#context_module_blank')
        .clone(true)
        ...
        function(data) {
          ...
          var nextProgression = function() {
            ...
            if (progression.user_id == window.ENV.current_user_id) {
              var $user_progression = $user_progression_list.find(
    ...
    editModule: function($module) {
      ...
      var data = $module.getTemplateData({
        textValues: [
          'name',
          'unlock_at',
    ...
    evaluateItemCyoe: function($item, data) {
      ...
      if (cyoe.isReleased) {
        ...
        var $srPath = $('<span class="screenreader-only">').append(htmlEscape(fullText))
    ...
    updateProgressionState: function($module) {
      ...
      var data = $progression.getTemplateData({
      ...
      var progression_state_capitalized =
        progression_state &&
    ...
    initMasterCourseLockButton: function($item, tagRestriction) {
      ...
      var isMasterCourseMasterContent = !!(
        'moduleItemId' in data && ENV.MASTER_COURSE_SETTINGS.IS_MASTER_COURSE
      )
      var isMasterCourseChildContent = !!(
        'moduleItemId' in data && ENV.MASTER_COURSE_SETTINGS.IS_CHILD_COURSE
      )
      var restricted = !!(
        'moduleItemId' in data &&
        Object.keys(tagRestriction).some(function(r) {
          return tagRestriction[r]
        })
      )
...
var updatePrerequisites = function($module, prereqs) {
  ...
  if (prereqs.length > 0) {
    ...
    var $prerequisitesMessage = $('<div />', {
      text: 'Prerequisites: ' + prereqsList,
...
modules.initModuleManagement = function() {
  ...
  $('#add_context_module_form .add_completion_criterion_link').click(function(event) {
    ...
    var $form = $(this).parents('#add_context_module_form')
    var $module = $form.data('current_module')
    var $option = $('#completion_criterion_option')
      .clone(true)
      .removeAttr('id')
  ...
  $('#completion_criterion_option .id').change(function() {
    ...
    var data = $('#context_module_item_' + $(this).val()).getTemplateData({
  ...
  $('.delete_item_link').live('click', function(event) {
    event.preventDefault()
    var $currentCogLink = $(this)
      .closest('.cog-menu-container')
      .children('.al-trigger')
    // Get the previous cog item to focus after delete
    var $allInCurrentModule = $(this)
      ...
      options.submit = function(item_data) {
        ...
        $module.disableWhileLoading(
          $.ajaxJSON(url, 'POST', item_data, function(data) {
            ...
            $item
              .find('.lock-icon')
              .data({moduleType: data.content_tag.type, contentId: data.content_tag.id})
  ...
  var overrideModuleModel = function(model) {
    ...
    model.publish = function() {
      return publish.apply(model, arguments).done(function(data) {
        ...
        model.fetch({data: {include: 'items'}}).done(function(attrs) {
          for (var i = 0; i < attrs.items.length; i++) updateModuleItem(attrs.items[i], model)
        })
      })
    }
    ...
    modules.updateProgressions(function() {
      if (window.location.hash && !window.location.hash.startsWith('#!')) {
        ...
        if ($('.context_module:first .content:visible').length == 0) {
          scrollTo(
            $('.context_module .content:visible')
              .filter(':first')
              .parents('.context_module')
          )
        }
