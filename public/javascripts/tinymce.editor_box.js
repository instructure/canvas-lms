...
function fillViewportWithEditor(editorID, elementToLeaveInViewport) {
  ...
  if ($iframe.length) {
    var newHeight =
      $(window).height() - ($iframe.offset().top + elementToLeaveInViewport.height() + 1)
...
function EditorBox(id, search_url, submit_url, content_url, options) {
  ...
  $textarea.bind('blur change', function() {
    if (
      $instructureEditorBoxList._getEditor(id) &&
      $instructureEditorBoxList._getEditor(id).isHidden()
    ) {
