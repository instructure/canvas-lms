...
export const quiz = (window.quiz = {
  ...
  checkShowDetails: function() {
    var hasQuestions = this.$questions.find(
      ...
      if (question.answers.length > 0) {
        ...
        $.each(question.answers, function(i, data) {
          ...
          if (question.answerDecimalPoints || question.answer_tolerance) {
            ...
            answerHtml =
              answerHtml + " <span style='font-size: 0.8em;'>+/-</span> " + htmlEscape(tolerance)
  ...
  updateFormQuestion: function($form) {
    ...
    if ($answers.length === 0 && result.answer_type != 'none') {
      $formQuestion
        .find('.form_answers')
        .append(makeFormAnswer({answer_type: result.answer_type, question_type: question_type}))
      $formQuestion
        .find('.form_answers')
        .append(makeFormAnswer({answer_type: result.answer_type, question_type: question_type}))
...
function makeFormAnswer(data) {
  var answer = $.extend({}, quiz.defaultAnswerData, data)
  var $answer = $('#form_answer_template')
    .clone(true)
    .attr('id', '')
  $answer
    .find('.answer_type')
  ...
  $list.each(function(i) {
    ...
    if (question.question_type != 'calculated_question') {
      ...
      question.formula_decimal_places =
        parseInt($question.find('.formula_decimal_places').text(), 10) || 0
      question.answer_tolerance =
        parseFloatOrPercentage($question.find('.answer_tolerance').text(), 10) || 0
    }
    question.position = i
...
$(document).ready(function() {
  ...
  $('#quiz_assignment_id')
    .change(function(event) {
      var previousData = $('#quiz_options').getTemplateData({
      ...
      var gradedQuiz = event.target.value === 'assignment' || event.target.value === 'graded_survey'
  ...
  $(document).delegate('.edit_question_link', 'click', function(event) {
    ...
    var $form = $('#question_form_template')
      .clone(true)
      .attr('id', '')
    var $formQuestion = $form.find('.question')
      ...
      for (var idx in question.answers) {
        ...
        if (question.answer_tolerance) {
          html =
            ...
            htmlEscape(question.answer_tolerance)
        }
        var $td = $("<td class='final_answer'/>")
        $td.html(html)
    ...
    if (REGRADE_OPTIONS[questionID]) {
      var regradeOption = $(QuizRegradeView.prototype.template()).find(
  ...
  $('.add_question_group_link').click(function(event) {
    ...
    var $group_top = $('#group_top_template')
      .clone(true)
      .attr('id', 'group_top_new')
    var $group_bottom = $('#group_bottom_template')
      .clone(true)
  ...
  var showQuestions = function(questionData) {
    ...
    $('.display_question:visible').each(function() {
      var id = $(this).getTemplateData({textValues: ['assessment_question_id']})
        .assessment_question_id
      if (id) {
        existingIDs[id] = true
      }
    ...
    for (var idx in questionList) {
      ...
      if (!existingIDs[question.id] || true) {
        ...
        var $question = $findQuestionDialog
  ...
  $('#question_form_template').submit(function(event) {
    ...
    if ($question.hasClass('calculated_question')) {
      ...
      question.formula_decimal_places =
        parseInt($question.find('.decimal_places .round').val(), 10) || 0
      ...
      question.answerDecimalPoints =
        parseFloat($question.find('.combination_error_margin').val(), 10) || 0
  ...
  $(document)
    .delegate('input.float_value', 'keydown', function(event) {
  ...
  $(document)
    .delegate('input.precision_value', 'keydown', function(event) {
  ...
  $(document)
    .delegate('input.combination_answer_tolerance', 'keydown', function(event) {
  ...
  $('#questions')
    .delegate('.question_teaser_link', 'click', function(event) {
      ...
      function showQuestion(question_data) {
        var $question = $('#question_template')
          .clone()
          .removeAttr('id')
  ...
  $('.quiz_group_form').formSubmit({
    ...
    success: function(data) {
      ...
      if ($('#insufficient_count_warning_' + group.id).length == 0) {
        var $warning = $('#insufficient_count_warning_template')
          .clone(true)
  ...
  var accessibleSortables = {
    ...
    itemsInGroup: function(group) {
      return $.grep(
        this.items,
        function(item) {
          return item.id != this.selected.id && (group == 'top' ? item.top : item.group == group)
        }.bind(this)
      )
    },
    ...
    showDialog: function() {
      ...
      this.$form.find('#move_quiz_item_cancel_btn').on(
        'click keyclick',
        function() {
  ...
  $('#questions').sortable({
    ...
    stop: function(event, ui) {
      ...
      var url = $('#quiz_urls .reorder_questions_url, #bank_urls .reorder_questions_url').attr(
        'href'
      )
      var data = {}
...
$.fn.formulaQuestion = function() {
  ...
  $question.find('.compute_combinations').click(function() {
    ...
    var answer_tolerance = parseFloatOrPercentage(
      $question.find('.combination_answer_tolerance').val()
    )
    var next = function() {
      $button.text(
              ...
              if ($variable.length === 0) {
                ...
                $variable = $(
                  "<tr class='variable'>" +
                    ...
                    label_id +
                    " equation_var_minimum' type='text' name='min' class='min variable_setting' style='width: 70%;' value='1'/></div></td>" +
                    "<td><div><input aria-labelledby='" +
                    label_id +
                    " equation_var_maximum' type='text' name='max' class='max variable_setting' style='width: 70%;' value='10'/></div></td>" +
                    "<td><div style='width: 70%;'><select aria-labelledby='" +
                    label_id +
                    " equation_var_precision' name='round' class='round variable_setting'><option>0</option><option>1</option><option>2</option><option>3</option></div></td>" +
