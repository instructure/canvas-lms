(function() {
  ...
  I18n.scoped('gradebook2', function(I18n) {
    ...
    return this.curveGradesDialog = curveGradesDialog = (function() {
      function curveGradesDialog(assignment, gradebook) {
        this.assignment = assignment;
        this.gradebook = gradebook;
        this.curve = __bind(this.curve, this);
        if (!this.$curve_grade_dialog) {
          this.initDialog();
        }
        this.$curve_grade_dialog.find(".assignment_title").text(this.assignment.name);
        this.$curve_grade_dialog.find(".assignment_id").val(this.assignment.id);
        this.$curve_grade_dialog.find(".out_of").showIf(this.assignment.points_possible >= 0);
        this.$curve_grade_dialog.find("#middle_score").val(parseInt((this.assignment.points_possible || 0) * 0.6));
        this.$curve_grade_dialog.find(".points_possible").text(this.assignment.points_possible);
        this.$curve_grade_dialog.dialog('option', 'title', I18n.t('curve_grade_for_course', "Curve Grade for %{assignment}", {
          'assignment': this.assignment.name
        })).dialog('open');
      }
      curveGradesDialog.prototype.initDialog = function() {
        curveGradesDialog.prototype.$curve_grade_dialog = $('#curve_grade_dialog');
        this.$curve_grade_dialog.formSubmit({
          ...
          processData: __bind(function(data) {
            ...
            if (cnt === 0) {
              this.$curve_grade_dialog.errorBox(I18n.t("errors.none_to_update", "None to Update"));
              return false;
            }
            return data;
          }, this),
          success: __bind(function(data) {
            ...
            this.$curve_grade_dialog.dialog('close');
        ...
        this.$curve_grade_dialog.find("#middle_score").bind("blur change keyup focus", this.curve);
        return this.$curve_grade_dialog.find("#assign_blanks").change(this.curve);
      };
      curveGradesDialog.prototype.curve = function() {
        ...
        idx = 0;
        scores = {};
        data = this.$curve_grade_dialog.getFormData();
        users_for_score = [];
        scoreCount = 0;
