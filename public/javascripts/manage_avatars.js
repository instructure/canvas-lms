...
$(document).ready(function() {
  $(".update_avatar_link").live('click', function(event) {
    ...
    if($link.attr('data-state') == 'none') {
      var result = confirm("Are you sure you want to delete this user's profile pic?");
      if(!result) { return; }
    } else if($link.attr('data-state') == 'locked') {
      var result = confirm("Locking this picture will approve it and prevent the user from updating it.  Continue?");
      if(!result) { return; }
    }
    var $td = $link.parents("td");
    var url = $td.find(".user_avatar_url").attr('href');
    $td.find(".progress").text("Updating...").css('visibility', 'visible');
    $.ajaxJSON(url, 'PUT', {'avatar[state]': $link.attr('data-state')}, function(data) {
      ...
      $td.find(".progress").text("Update failed, please try again").css('visibility', 'visible');
    });
  });
});