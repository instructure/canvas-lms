...
var rubricEditing = {
  ...
  updateAddCriterionLinks($rubric, forceFocus = false) {
    if ($rubric.find('#add_criterion_holder').length === 0) {
      return
    }
  ...
  addNewRatingColumn: function($this) {
    ...
    if ($rubric.hasClass('editing')) {
      ...
      if ($this.hasClass('add_left')) {
        var more_points = numberHelper.parse(
          $this
            .prev('.rating')
            .find('.points')
            .text()
        ...
        var less_points = numberHelper.parse(
          $this
            .next('.rating')
            .find('.points')
            .text()
  ...
  onFindOutcome: function(outcome) {
    ...
    $criterion
      ...
      .val(outcome.get('ratings')[0].points)
      .blur()
  ...
  updateCriterionPoints: function($criterion, baseOnRatings) {
    ...
    if (
      ...
      if (oldMax !== newMax) {
        ...
        $($ratingList[0])
          .find('.points')
          .text(rubricEditing.localizedPoints(points))
        var lastPts = points
        ...
        for (var i = 1; i < $ratingList.length - 1; i++) {
          ...
          $($ratingList[i])
            .find('.points')
            .text(rubricEditing.localizedPoints(newPts))
  ...
  rubricData: function($rubric) {
    ...
    $rubric.find('.criterion:not(.blank)').each(function() {
      ...
      var vals = $criterion.getTemplateData({
        textValues: [
          'description',
  ...
  editRubric: function($original_rubric, url) {
    ...
    data = $rubric.getTemplateData({
      textValues: ['use_for_grading', 'free_form_criterion_comments', 'hide_score_total']
    })
  ...
  updateRubric: function($rubric, rubric) {
    ...
    rubric.criteria.forEach(function(criterion) {
      ...
      criterion.ratings.forEach(function(rating) {
        ...
        $rating
          .find('.range_rating')
          .showIf(criterion.criterion_use_range === true && rating.min_points !== rating.points)
...
rubricEditing.init = function() {
  ...
  $rubric_long_description_dialog.find('.save_button').click(function() {
    ...
    if ($criterion) {
      $criterion.fillTemplateData({
        data: {long_description: long_description, criterion_description_value: description}
      })
  ...
  $('#rubric_dialog')
        ...
        $.ajaxJSON(
          ...
          function(data) {
            ...
            data.forEach(function(item) {
              ...
              var $rubric_select = $rubric_dialog
    ...
    .delegate('.select_rubric_link', 'click', function(event) {
      event.preventDefault()
      var data = {}
      var params = $rubric_dialog.getTemplateData({
        textValues: [
  ...
  $('#edit_rubric_form').formSubmit({
    ...
    success: function(data, $rubric) {
      ...
      if (
        ...
        var discussion_points_text = I18n.t(
  ...
  $('#rubrics')
    .delegate('.add_criterion_link', 'click', function(event) {
