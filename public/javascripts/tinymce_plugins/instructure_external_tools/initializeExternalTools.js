...
const TRANSLATIONS = {
  embed_from_external_tool: I18n.t(
    'embed_from_external_tool',
    '"Embed content from External Tool"'
  ),
...
var ExternalToolsPlugin = {
  init(ed, url, _INST) {
    ...
    for (let idx = 0; _INST.editorButtons && idx < _INST.editorButtons.length; idx++) {
      ...
      if (
        ...
        ;(function(button) {
          ed.addCommand(`instructureExternalButton${button.id}`, () => {
            ExternalToolsPlugin.buttonSelected(button, ed)
          })
          ed.addButton(
            `instructure_external_button_${button.id}`,
            ExternalToolsHelper.buttonConfig(button)
          )
        })(current_button)
      }
    }
    if (clumpedButtons.length) {
      const handleClick = function() {
        const items = ExternalToolsHelper.clumpedButtonMapping(
          clumpedButtons,
          ed,
          ExternalToolsPlugin.buttonSelected
        )
  ...
  buttonSelected(button, ed) {
    let $dialog = $('external_tool_button_dialog')
    ...
    if (!$dialog.length) {
      ...
      const dialogHTML =
        '<div id="external_tool_button_dialog" style="padding: 0; overflow-y: hidden;"/>'
      const iframeAllowancesString = iframeAllowances()
      $dialog = $(dialogHTML)
        .hide()
        .html(
          `${"<div class='teaser' style='width: 800px; margin-bottom: 10px; display: none;'></div>" +
            ...
            '<i class="icon-info"></i>' +
            '</div>'}${htmlEscape(I18n.t('The following content is partner provided'))}</div>` +
            `</div>` +
            ...
            `</form>` +
            `<iframe name='external_tool_launch' src='/images/ajax-loader-medium-444.gif' id='external_tool_button_frame' style='width: 800px; height: ${frameHeight}px; border: 0;' allow='${iframeAllowancesString}' borderstyle='0' tabindex='0'/>` +
            `<div class="after_external_content_info_alert screenreader-only" tabindex="0">` +
            `<div class="ic-flash-info">` +
            `<div class="ic-flash__icon" aria-hidden="true">` +
            `<i class="icon-info"></i>` +
            `</div>${htmlEscape(I18n.t('The preceding content is partner provided'))}</div>` +
            `</div>`
        )
        .appendTo('body')
        .dialog({
    ...
    deepLinkingListener =
      deepLinkingListener ||
      ExternalToolsHelper.createDeepLinkingListener(
    ...
    $(window).bind('externalContentReady', (event, data) => {
      let editor = $dialog.data('editor') || ed,
    ...
    $dialog.dialog({
      ...
      open: () =>
        ExternalToolsHelper.contentItemDialogOpen(
          button,
          ed,
