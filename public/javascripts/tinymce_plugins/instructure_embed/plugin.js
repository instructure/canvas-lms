...
function initShared() {
  $box = $('<div/>', {
    html:
      htmlEscape(TRANSLATIONS.instructions) +
      "<form id='instructure_embed_prompt_form' style='margin-top: 5px;'><table class='formtable'><tr><td>" +
      htmlEscape(TRANSLATIONS.url) +
      "</td><td><input type='text' class='prompt' style='width: 250px;' value='http://'/></td></tr><tr><td class='nobr'>" +
      htmlEscape(TRANSLATIONS.alt_text) +
      "</td><td><input type='text' class='alt_text' style='width: 150px;' value=''/></td></tr><tr><td colspan='2' style='text-align: right;'><input type='submit' class='btn' value='Embed Image'/></td></tr></table></form><div class='actions'></div>"
  }).hide()
  ...
  $box
    .append($flickrLink)
    .find('#instructure_embed_prompt_form')
    .submit(embedURLImage)
  $('body').append($box)
  $box.dialog({
    autoOpen: false,
    width: 425,
...
function flickrLinkClickHandler(event) {
  ...
  $.findImageForService('flickr_creative_commons', function(data) {
    var title = data.title,
      html =
        '<a href="' +
        htmlEscape(data.link_url) +
        '"><img src="' +
        htmlEscape(data.image_url) +
        '" title="' +
        htmlEscape(title) +
        '"alt="' +
        htmlEscape(title) +
        '" style="max-width: 500; max-height: 500"></a>'
    $box.dialog('close')
