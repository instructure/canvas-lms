...
$.titleize = function(string) {
  var res = (string || '')
    ...
    .replace(/\s+/, ' ')
    .replace(/^\s/, '')
  return $.map(res.split(/\s/), function(word) {
  ...
  if (!$dialog.length) {
    ...
    $dialog.append(
      "<form id='bookmark_search_form' style='margin-bottom: 5px;'>" +
        ...
        htmlEscape(I18n.t('buttons.search', 'Search')) +
        '</button></form>'
    )
...
$.findImageForService = function(service_type, callback) {
  ...
  if (!$dialog.length) {
    $dialog = $("<div id='instructure_image_search'/>")
      .append(
        "<form id='image_search_form' class='form-inline' style='margin-bottom: 5px;'>" +
    ...
    $dialog.find('form').submit(function(event) {
        ...
        if (data && data.photos && data.photos.photo) {
          ...
          for (var idx in data.photos.photo) {
            ...
            $dialog.find('.results').append(
              $('<div class="image" style="float: left; padding: 2px; cursor: pointer;"/>').append(
                $('<img/>', {
                  data: {
                    source: source_url,
      ...
      $.getScript(
        'https://secure.flickr.com/services/rest/?method=flickr.photos.search&format=json&api_key=734839aadcaa224c4e043eaf74391e50&per_page=25&license=1,2,3,4,5,6&sort=relevance&text=' +
          query
      )
    })
  ...
  $dialog.dialog({
    title: I18n.t('titles.image_search', 'Image Search: %{service_name}', {
      service_name: $.titleize(service_type)
    }),
    width: 440,
    open: function() {
