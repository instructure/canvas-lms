...
var SubmitAssignment = {
  toolDropDownClickHandler: function(event) {
    ...
    var url =
      ...
      '/external_tools/' +
      tool.id +
      '/resource_selection?homework=1&assignment_id=' +
      ENV.SUBMIT_ASSIGNMENT.ID
    ...
    $div.append(
      $('<div/>', {id: 'tab-helper', style: 'height:0px;padding:5px', tabindex: '0'})
        .focus(function() {
          ...
          var joke = document.createTextNode(
...
$(document).ready(function() {
  ...
  var homeworkSubmissionLtiContainer = new HomeworkSubmissionLtiContainer(
  ...
  submissionForm.submit(function(event) {
    ...
    if ($(this).attr('id') == 'submit_online_upload_form') {
      ...
      var uploadedAttachmentIds = $(this)
      ...
      if (ENV.SUBMIT_ASSIGNMENT.ALLOWED_EXTENSIONS.length > 0) {
        ...
        $.each(uploadedAttachmentIds.split(','), function(index, id) {
          if (id.length > 0) {
            var ext = $('#uploaded_files .file_' + id + ' .name')
              .text()
              .split('.')
              .pop()
              .toLowerCase()
...
function uploadFileFromUrl() {
  ...
  $tools.disableWhileLoading(promise, {
...
$('#submit_from_external_tool_form .tools li').live(
  'click',
  SubmitAssignment.toolDropDownClickHandler
)
