...
function submissionLoaded(data) {
  ...
  for (var jdx in data) {
    ...
    if (submission.user_id != ENV.SUBMISSION.user_id) {
      continue
    }
    ...
    for (var idx in comments) {
      ...
      if (comment.media_comment_id) {
        var $media_comment_link = $('#comment_media_blank')
          .clone(true)
          .removeAttr('id')
        ...
        for (var jdx in comment.attachments) {
          ...
          var $attachment = $('#comment_attachment_blank')
            .clone(true)
            .removeAttr('id')
    ...
    $(this).bind('keydown', function(e) {
      if (keyCodes[e.which] == 'tab') {
        ...
        if (isLeavingHolder) {
          e.preventDefault()
          var thisEl = this
          var target = $.grep(tabBounds, function(el) {
            return el != thisEl
          })
          $(target).focus()
        }
...
export function setup() {
  $(document).ready(function() {
    ...
    $('.attach_comment_file_link').click(function(event) {
      event.preventDefault()
      var $attachment = $('#comment_attachment_input_blank')
        .clone(true)
        .removeAttr('id')
      ...
      .delegate('.play_comment_link', 'click', function(event) {
        ...
        if (comment_id) {
          $(this)
            ...
            .mediaComment('show', comment_id, 'video')
        }
      })
