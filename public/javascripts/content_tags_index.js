...
$(document).ready(function() {
  $('#tags .tag_name').click(function(event) {
    ...
    var name = $(this)
      .getTemplateData({textValues: ['name']})
      .name.toLowerCase()
      .replace(/\s/g, '_')
    $('#pages_for_tags')
      .find('.page')
  ...
  $(document).bind('tag_added', function(event, tag) {
    ...
    $page
      .find('.delete_page_link')
      .attr('href', $.replaceTags($page.find('.delete_page_link').attr('href'), 'id', tag.id))
    $page
      .addClass('page_for_' + tag.context_type.toLowerCase() + '_' + tag.context_id)
  ...
  $(window)
    .resize(function() {
      ...
      if ($.browser.msie && navigator.appVersion.match('MSIE 6.0')) {
        return
      }
