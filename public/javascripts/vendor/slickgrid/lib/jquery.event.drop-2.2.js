    ...
    drop = ($.event.special.drop = {
      ...
      modes: {
        ...
        overlap: function(event, proxy, target) {
          ...
          return (
            Math.max(0, Math.min(target.bottom, proxy.bottom) - Math.max(target.top, proxy.top)) *
      ...
      tolerate: function(dd) {
              ...
              if ((data = arr[i])) {
                ...
                if (data.winner && ia.drop.length < drop.multi) {
                  ...
                  if (!data.active[x] && !data.anyactive) {
                    ...
                    if (
                      $special.drag.hijack(drop.event, 'dropstart', dd, x, data.elem)[0] !== false
                    ) {
                      data.active[x] = 1
                      data.anyactive += 1
                    } else
        ...
        if (drop.last && xy[0] == drop.last.pageX && xy[1] == drop.last.pageY)
          delete drop.timer // idle, don't recurse // recurse
        else
