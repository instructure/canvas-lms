...
;(function(factory) {
  ...
  Selectpicker.prototype = {
      ...
      this.$element.find('option').each(function(index) {
        ...
        var subtext =
          $this.data('subtext') !== undefined
            ...
            : ''
        var icon =
          $this.data('icon') !== undefined ? '<i class="' + $this.data('icon') + '"></i> ' : ''
        ...
        if (!$this.data('content')) {
          ...
          text =
            icon + '<span class="text" data-value="' + value + '">' + text + subtext + '</span>'
        }

        if (
          _this.options.hideDisabled &&
            ...
            if (_this.options.useSubmenus) {
              _liA.push(
                '<div class="div-contain"><div class="divider"></div></div>' +
                  ...
                  label +
                  '</a>' +
                  '<div class="dropdown-menu open" data-content-type="' +
                  content_type +
                  '" tabindex="0"><ul class="dropdown-menu inner" role="group">'
              )
    ...
    createA: function(text, classes, inline, index, $option) {
      ...
      var $obj = $(
        '<a class="' + classes + '">' + text + '<i class="icon-ok check-mark"></i>' + '</a>'
      )
    ...
    render: function(inUse) {
      ...
      var selectedItems = this.$element
        ...
        .map(function(index, value) {
          var $this = $(this)
          var icon =
            $this.data('icon') && _this.options.showIcon
      ...
      if (_this.multiple && _this.options.selectedTextFormat.indexOf('count') > -1) {
        ...
        if (
          ...
          title = _this.options.countSelectedText
      ...
      if (!title) {
        title =
          _this.options.title != undefined ? _this.options.title : _this.options.noneSelectedText
      }
      ...
      if (this.options.showSubtext && this.$element.find('option:selected').attr('data-subtext')) {
        subtext =
          ' <small class="muted">' +
          this.$element.find('option:selected').data('subtext') +
          '</small>'
      } else {
        subtext = ''
      }
      ...
      // INSTRUCTURE
      if (inUse) {
        $('li:not(.divider):visible > a', _this.$newElement)
          .first()
          .focus()
      }
    },
    ...
    refresh: function() {
      ...
      var inUse =
        this.$newElement.hasClass('open') && $.contains(this.$menu[0], document.activeElement)
    ...
    keydown: function(e) {
      var $this, $items, $parent, index, next, first, last, prev, nextPrev, $target, $list
