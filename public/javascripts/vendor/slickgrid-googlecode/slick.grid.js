...
return function SlickGrid($container,data,columns,options){
		...
		$divHeaders.find(".h").each(function() {
			...
			$(this).resizable({
				...
				stop: function(e, ui) {
					...
					$('body').append("<style class='slick-style'> ." + uid + " .grid-canvas .c" + cell + "{width: " + columns[cell].width + "px;} </style>");
					resizeCanvas();
	...
	function createCssRules() {
		for (var i = 0; i < columns.length; i++) {
		  $('body').append("<style class='slick-style'> ." + uid + " .grid-canvas .c" + i + " { width:" + columns[i].width + "px } </style>");
		}
	}
	
	function removeCssRules() {
		$('body').find('style.slick_style').remove();
	}
	...
	function appendRowHtml(stringArray,row) {
		...
		for (var i=0, cols=columns.length; i<cols; i++) 
		{
			...
			stringArray.push("<div " + (m.unselectable ? "" : "hideFocus tabIndex=0 ") + "class='c c" + i + (m.cssClass ? " " + m.cssClass : "") + ( (m.active && d.active) ? " active-col-and-row" : "") + "' cell=" + i + ">");
