...
I18n.parseDate = function(d) {
  ...
  if (matches) {
    ...
    if (matches[7]) {
      date = new Date(
        Date.UTC(matches[1], matches[2], matches[3], matches[4], matches[5], matches[6])
      )
    } else {
...
I18n.toHumanSize = function(number, options) {
  ...
  options = this.prepareOptions(options, {precision: precision, format: '%n%u', delimiter: ''})
