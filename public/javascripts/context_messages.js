...
$(document).ready(function() {
  ...
  $("#mark_inbox_as_read_form").formSubmit({
    beforeSubmit: function(data) {
      $(this).find("button").attr('disabled', true)
        .find(".msg").text("Marking All as Read...");
    },
    success: function(data) {
      $("#message_list .inbox_item:visible").addClass('read');
      $(this).find("button").attr('disabled', false)
        .find(".msg").text("Mark All as Read");
      $("#identity .unread-messages-count").remove();
      $(this).slideUp();
    },
    error: function(data) {
      $(this).find("button").attr('disabled', false)
        .find(".msg").text("Marking failed, please try again");
    }
  });
  ...
  $(".send_message_form").formSubmit({
    ...
    processData: function(data) {
      ...
      if(recipients.length === 0) {
        $(this).errorBox("Please select at least one recipient");
        return false;
      }
    ...
    beforeSubmit: function() {
      $(this).loadingImage();
      $(this).find(".send_button").attr('disabled', true).text("Sending Message...");
      $(this).find(".cancel_button").attr('disabled', true);
    },
    success: function(data) {
      $(this).find(".send_button").attr('disabled', false).text("Send Message");
      $(this).find(".cancel_button").attr('disabled', false);
      $(this).loadingImage('remove');
      if($("#messages_view").text() == "Sentbox") {
  ...
  $(".send_message_form .select_recipients").click(function() {
    ...
    if($dialog.hasClass('loaded')) {
      ...
      $dialog.children(".left_side").append("<div class='message'>Loading Recipients List...</div>");
  ...
  $(".context_message").find(".reply_link,.reply_to_all_link").click(function(event) {
    ...
    var subject = $message.getTemplateData({textValues: ['subject']}).subject;
    ...
    $form.fillFormData({subject: "Re: " + subject, root_context_message_id: root_id}, {object_name: 'context_message'});
    $message = $message.clone(true).removeClass('read');
    $message.find(".link_box,.sub_title,.show_recipients_link").remove();
    $message.find(".subject").text("Original Message"); //$message.find(".subject")
