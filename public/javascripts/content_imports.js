...
$(function() {
  ...
  var populateItem = function($item, id, name, param_name) {
    ...
    setTimeout(function() {
      if ($item != null) {
        ...
        if (name != null) {
          $item.find('label:first').text(name)
        }
        $item.find('label:first').attr('for', full_param_name.replace(/[\[\]]+/g, '_'))
        $item.show()
      }
      pendingPopulates -= 1
  ...
  $.ajaxJSON(location.href, 'GET', {}, function(data) {
    ...
    if (data.assessments && data.assessments) {
      for (var idx in data.assessments) {
        ...
        populateItem(
          $quiz,
          quiz.assessment_id || quiz.migration_id,
          quiz.assessment_title || quiz.quiz_name,
          'copy[quizzes]'
        )
    ...
    $('#copy_assignment_groups_list').showIf(
      data.assignment_groups && data.assignment_groups.length > 0
    )
    if (data.assignment_groups) {
      for (var idx in data.assignment_groups) {
        var assignment_group = data.assignment_groups[idx]
        var $assignment_group = $('#copy_assignment_groups_list .assignment_group:first').clone(
          true
        )
        populateItem(
          $assignment_group,
          assignment_group.migration_id,
          assignment_group.title,
          'copy[assignment_groups]'
        )
  ...
  $('#copy_context_form :checkbox').change(function() {
    if ($(this).hasClass('copy_all')) {
      $(this)
        .parent()
        .nextAll('ul')
        .find(':checkbox:not(.secondary_checkbox)')
      ...
      $(
        '#copy_context_form :checkbox:not(.secondary_checkbox):not(.copy_everything):not(.shift_dates_checkbox)'
      )
        .prop('checked', $(this).prop('checked'))
        .filter(':not(.copy_all)')
        .each(function() {
      ...
      $(this)
        .parent()
        .find(':checkbox.secondary_checkbox')
        .prop('checked', $(this).prop('checked'))
  ...
  $('.add_substitution_link').click(function(event) {
    ...
    var $sub = $('.substitution_blank')
      .clone(true)
      .removeClass('substitution_blank')
    $('.substitutions').append($sub.hide())
    var $select = $('.weekday_select_blank')
      .clone(true)
      .removeClass('weekday_select_blank')
    $sub
      .find('.old_select')
      .empty()
      .append($select.clone(true))
    $sub
      .find('.new_select')
      .empty()
      .append($select)
  ...
  $('#copy_context_form').formSubmit({
    ...
    beforeSubmit: function(data) {
      setTimeout(function() {
        $('#copy_context_form')
          .find(':checkbox,:text,select')
          .attr('disabled', true)
      }, 1000)
      $('#copy_context_form .submit_button')
        .text(I18n.t('messages.importing_button', 'Importing... this could take a while'))
        .attr('disabled', true)
        ...
        $.ajaxJSON(
          ...
          function(data) {
            ...
            if (course_import && course_import.workflow_state == 'imported') {
              $.flashMessage(
                I18n.t(
                  'messages.import_complete',
                  'Import Complete!  Returning to the Course Page...'
                )
              )
              ...
              var code =
                'ContentMigration:' +
                $('.content_migration_id:first').text() +
                ':' +
                course_import.progress
              var message = I18n.t(
                'errors.import_failed',
                'There was an error during your migration import.  Please notify your system administrator and give them the following code: "%{code}"',
                {code: code}
              )
              $.flashError(message)
