$(document).ready(function() {
  ...
  $(".grading_standard .delete_grading_standard_link").click(function(event) {
    ...
    $standard.confirmDelete({
      url: url,
      message: "Are you sure you want to delete this grading scheme?",
      ...
      error: function() {
        $.flashError("There was a problem deleting this grading scheme");
      }
    });
  });
  $(".grading_standard .remove_grading_standard_link").click(function(event) {
    event.preventDefault();
    var result = confirm("Are you sure you want to unlink this grading scheme?");
    ...
    function removed() {
      ...
      $("#course_form .grading_scheme_set").text("Not Set");
    ...
    if(url) {
      $.ajaxJSON(url, 'PUT', put_data, removed, function() {
        $.flashError("There was a problem removing this grading scheme.  Please reload the page and try again.");
      });
    } else {
      removed();
    }
    ...
    if(standard.id) {
      ...
      if(url) {
        $.ajaxJSON(url, 'PUT', put_data, function(data) {
          $("#course_form .grading_scheme_set").text((data && data.course && data.course.grading_standard_title) || "Currently Set");
        }, function() {});
      }
    } else {
  ...
  $(".grading_standard .save_button").click(function(event) {
    ...
    $standard.find("button").attr('disabled', true).filter(".save_button").text("Saving...");
    $.ajaxJSON(url, method, data, function(data) {
      var standard = data.grading_standard;
      $standard.find("button").attr('disabled', false).filter(".save_button").text("Save");
      $standard.triggerHandler('grading_standard_updated', standard);
    }, function() {
      $standard.find("button").attr('disabled', false).filter(".save_button").text("Save Failed");
    });
  });
