...
$(document).ready(function() {
  ...
  $('#select_context_content_dialog .add_item_button').click(function() {
    ...
    if (item_type == 'external_url') {
      ...
      if (item_data['item[url]'] === '') {
        var $errorBox = $('<div />', {class: 'alert alert-error', role: 'alert'}).css({
          marginTop: 8
        })
        $errorBox.text(
  ...
  $('#add_module_item_select').change(function() {
    ...
    var doNotDisable = _.contains(
      ['external_url', 'context_external_tool', 'context_module_sub_header'],
      selectedOption
    )
    if (doNotDisable) {
      $('.add_item_button')
        ...
        $.ajaxJSON(
          ...
          function(data) {
            ...
            for (var idx in data) {
              ...
              $tool.toggleClass(
                'resource_selection',
                'resource_selection' in tool.placements ||
                  placement.message_type == 'ContentItemSelectionRequest'
              )
