...
_.initBlocks = function(replacedFragment) {
  ...
  if (self.html_template.length === 1) {
    self.firstChild = self.lastChild = self.jQ.data(jQueryDataKey).block =
...
_.placeCursor = function(cursor) {
  ...
  cursor.appendTo(
    this.foldChildren(this.firstChild, function(prev, child) {
...
function addToolbar(root, jQ) {
  ...
  var html_template_overrides = {
    ...
    frac:
      '<span style="font-size: 0.55em; vertical-align: middle" class="fraction"><span class="numerator"><var>n</var></span><span class="denominator"><var>m</var></span><span style="width:0"></span></span>',
    sqrt:
      '<span class="block"><span class="sqrt-prefix">&radic;</span><span class="sqrt-stem">&nbsp;</span></span>',
    nthroot:
    ...
    case 'U+0009':
      ...
      if (e.shiftKey) {
        ...
        if (parent === this.cursor.root)
          ...
          //go one block left
          this.cursor.appendTo(parent.prev) //get out of the block
        ...
        if (parent === this.cursor.root)
          ...
          //go one block right
          this.cursor.prependTo(parent.next) //get out of the block
    ...
    case 'End':
      if (e.shiftKey)
        while (this.cursor.next || (e.ctrlKey && this.cursor.parent !== this))
          this.cursor.selectRight() //move to the end of the root block or the current block.
    ...
    case 'Home':
      if (e.shiftKey)
        while (this.cursor.prev || (e.ctrlKey && this.cursor.parent !== this))
          this.cursor.selectLeft() //move to the start of the root block or the current block.
...
LatexCmds.superscript = LatexCmds.supscript = LatexCmds['^'] = proto(SupSub, function(
  replacedFragment
) {
...
_.placeCursor = function(cursor) {
  ...
  if (this.replacedText)
    for (var i = 0; i < this.replacedText.length; i += 1) this.write(this.replacedText.charAt(i))
}
_.write = function(ch) {
...
LatexCmds.alpha = LatexCmds.beta = LatexCmds.gamma = LatexCmds.delta = LatexCmds.zeta = LatexCmds.eta = LatexCmds.theta = LatexCmds.iota = LatexCmds.kappa = LatexCmds.mu = LatexCmds.nu = LatexCmds.xi = LatexCmds.rho = LatexCmds.sigma = LatexCmds.tau = LatexCmds.chi = LatexCmds.psi = LatexCmds.omega = proto(
  Symbol,
  function(replacedFragment, latex) {
...
LatexCmds.Gamma = LatexCmds.Delta = LatexCmds.Theta = LatexCmds.Lambda = LatexCmds.Xi = LatexCmds.Pi = LatexCmds.Sigma = LatexCmds.Phi = LatexCmds.Psi = LatexCmds.Omega =
...
LatexCmds.notin = LatexCmds.sim = LatexCmds.cong = LatexCmds.equiv = LatexCmds.oplus = LatexCmds.otimes = proto(
  BinaryOperator,
  function(replacedFragment, latex) {
...
;(function() {
  ...
  for (var i in trig) {
    LatexCmds[trig[i]] = LatexCmds[trig[i] + 'h'] = LatexCmds['a' + trig[i]] = LatexCmds[
      'arc' + trig[i]
    ] = LatexCmds['a' + trig[i] + 'h'] = LatexCmds['arc' + trig[i] + 'h'] = NonItalicizedFunction
  }
})()
    ...
    while (latex.length) {
      ...
      if (token.slice(0, 6) === '\\text{') {
        ...
        if (cursor.prev)
          ...
          return //was an open-paren, hack to put the following latex
