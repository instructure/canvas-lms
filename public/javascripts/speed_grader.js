...
function mergeStudentsAndSubmission() {
  ...
  if (hideStudentNames && userSettings.get('eg_sort_by') === 'alphabetically') {
    window.jsonData.studentsWithSubmissions.sort(
      EG.compareStudentsBy(function(student) {
    ...
    window.jsonData.studentsWithSubmissions.sort(
      EG.compareStudentsBy(function(student) {
        var submittedAt = student && student.submission && student.submission.submitted_at
        if (submittedAt) {
    ...
    window.jsonData.studentsWithSubmissions.sort(
      EG.compareStudentsBy(function(student) {
        return student && states[SpeedgraderHelpers.submissionState(student, ENV.grading_role)]
      })
    )
  }
}
...
function beforeLeavingSpeedgrader(e) {
  ...
  window.opener &&
    ...
    $.isFunction(window.opener.updateGrades) &&
    window.opener.updateGrades()

  var userNamesWithPendingQuizSubmission = $.map(snapshotCache, function(snapshot) {
    return (
      snapshot &&
      $.map(jsonData.context.students, function(student) {
  ...
  var hasPendingQuizSubmissions = (function() {
    ...
    if (userNamesWithPendingQuizSubmission.length) {
      for (var i = 0, max = userNamesWithPendingQuizSubmission.length; i < max; i++) {
        if (userNamesWithPendingQuizSubmission[i] !== false) {
          ret = true
        }
      }
    }
...
EG = {
  ...
  domReady: function() {
    ...
    $new_mark_link.click(function(e) {
      e.preventDefault()
      EG.newProvisionalGrade('new', 1)
    })
    $new_mark_final_link.click(function(e) {
      e.preventDefault()
      EG.newProvisionalGrade('new', 'final')
    })
    $new_mark_copy_link1.click(function(e) {
      e.preventDefault()
      EG.newProvisionalGrade('copy', 0)
    })
    $new_mark_copy_link2.click(function(e) {
      e.preventDefault()
      EG.newProvisionalGrade('copy', 1)
    })
  ...
  skipRelativeToCurrentIndex: function(offset) {
    var newIndex =
      (this.currentIndex() + offset + jsonData.studentsWithSubmissions.length) %
      jsonData.studentsWithSubmissions.length
  ...
  refreshFullRubric: function() {
    ...
    rubricAssessment.populateRubric(rubricFull.find('.rubric'), rubricAssessmentToPopulate())
  ...
  handleFragmentChange: function() {
    ...
    var studentId = jsonData.context.rep_for_student[hash.student_id] || hash.student_id
  ...
  handleModerationTabs: function(index_to_load) {
    var prov_grades =
      this.currentStudent.submission && this.currentStudent.submission.provisional_grades
    var final_grade =
      this.currentStudent.submission && this.currentStudent.submission.final_provisional_grade
  ...
  updateModerationTabs: function() {
    ...
    this.updateModerationTab(
      $moderation_tab_final,
      this.currentStudent.submission.final_provisional_grade
    )
  },
  ...
  updateModerationTab: function($tab, prov_grade) {
    ...
    var $srMessage = $('<span class="selected_sr_message screenreader-only"></span>').text(
      CHOSEN_GRADE_MESSAGE
    )
    ...
    if (prov_grade && prov_grade.score) {
      $mark_grade.html(
        htmlEscape(I18n.n(prov_grade.score) + '/' + I18n.n(window.jsonData.points_possible))
      )
    } else {
      $mark_grade.empty()
    }
  ...
  newProvisionalGrade: function(type, index) {
    if (type == 'new') {
      ...
      if (
        ...
        confirm(
          I18n.t(
            'Are you sure you want to copy to the final mark? This will overwrite the existing mark.'
          )
        )
      ) {
        ...
        $full_width_container.disableWhileLoading(
          $.ajaxJSON(
            $.replaceTags(ENV.provisional_copy_url, {
              provisional_grade_id: grade_to_copy.provisional_grade_id
            }),
            'POST',
            {},
            function(data) {
              $.each(EG.currentStudent.submission.provisional_grades, function(i, pg) {
                pg.selected = false
              })
  ...
  selectProvisionalGrade: function(index) {
    ...
    $full_width_container.disableWhileLoading(
      $.ajaxJSON(
        $.replaceTags(ENV.provisional_select_url, {
          provisional_grade_id: prov_grade.provisional_grade_id
        }),
        'PUT',
        {},
        function(data) {
  ...
  populateTurnitin: function(
    ...
    if (
      ...
      var urlContainer = SpeedgraderHelpers.urlContainer(
        submission,
      ...
      $turnitinScoreContainer.html(
        turnitinScoreTemplate({
          ...
          reportUrl: $.replaceTags(urlContainer.attr('href'), {
    ...
    if (
      ...
      $vericiteScoreContainer.html(
        vericiteScoreTemplate({
          ...
          tooltip: I18n.t(
            'vericite.tooltip.score',
            'VeriCite Similarity Score - See detailed report'
          ),
  ...
  handleSubmissionSelectionChange: function() {
    ...
    var $submission_to_view = $('#submission_to_view'),
      ...
      submissionHolder = this.currentStudent && this.currentStudent.submission,
      submissionHistory = submissionHolder && submissionHolder.submission_history,
      isMostRecent = submissionHistory && submissionHistory.length - 1 === currentSelectedIndex,
      submission =
        (submissionHistory &&
  ...
  refreshSubmissionsToView: function() {
    ...
    if (s && s.submission_history && s.submission_history.length > 0) {
      ...
      innerHTML = submissionsDropdownTemplate({
        ...
        quizHistoryHref: $.replaceTags(ENV.quiz_history_url, {user_id: this.currentStudent.id})
      })
    }
  ...
  renderSubmissionPreview: function() {
    ...
    var src = htmlEscape(
      '/courses/' +
        jsonData.context_id +
        '/assignments/' +
        this.currentStudent.submission.assignment_id +
        '/submissions/' +
        this.currentStudent.submission.user_id +
        '?preview=true' +
  ...
  attachmentIFrameContents(attachment) {
    ...
    if (attachment.mime_class === 'image') {
      ...
      contents = SpeedgraderHelpers.buildIframe(htmlEscape(src), {
        frameborder: 0,
        allowfullscreen: true
      })
    }
  ...
  showRubric: function() {
    ...
    if (jsonData.rubric_association) {
      ...
      $.each(this.currentStudent.rubric_assessments, function() {
        selectMenu.append(
          `<option value="${htmlEscape(this.id)}">${htmlEscape(this.assessor_name)}</option>`
        )
      })
  ...
  addCommentSubmissionHandler: function(commentElement, comment) {
    ...
    // this comment... and if the student isn't concluded
    var isConcluded = EG.isStudentConcluded(EG.currentStudent.id)
    var commentIsPublishableByMe =
      comment.draft && comment.author_id.toString() === ENV.current_user_id && !isConcluded
  ...
  updateSelectMenuStatus: function(student) {
    ...
    $selectmenu.updateSelectMenuStatus(
      student,
      isCurrentStudent,
      EG.getStudentNameAndGrade(student)
    )
  },
...
function getAssignmentOverrides() {
  return $.getJSON(
    '/api/v1/courses/' + ENV.course_id + '/assignments/' + ENV.assignment_id + '/overrides'
  )
}
