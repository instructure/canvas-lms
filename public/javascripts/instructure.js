      ...
      .each(function() {
        ...
        if ($.trim($link.text())) {
          var $span = $("<span class='instructure_scribd_file_holder link_holder'/>"),
            $scribd_link = $(
              "<a class='scribd_file_preview_link' aria-hidden='true' tabindex='-1' href='" +
...
$(function() {
  ...
  function resizeBreadcrumb() {
    ...
    $breadcrumbEllipsis.ifExists(function() {
      for (var i = 0; $breadcrumbs.height() > hightOfOneBreadcrumb && i < 20; i++) {
  ...
  $(document)
    .bind('richTextStart', function(event, $editor) {
  ...
  $('.communication_sub_message .add_sub_message_form').formSubmit({
    ...
    success: function(data) {
      ...
      if ($(this).hasClass('submission_comment_form')) {
        var user_id = $(this).getTemplateData({textValues: ['submission_user_id']})
          .submission_user_id
        var submission = null
        for (var idx in data) {
  ...
  var sequence_url = $('#sequence_footer .sequence_details_url')
    .filter(':last')
    .attr('href')
  if (sequence_url) {
  ...
  setTimeout(function() {
    $('#content a:external,#content a.explicit_external_link').each(function() {
      ...
      var $linkIndicator = $('<span class="ui-icon ui-icon-extlink ui-icon-inline"/>').attr(
        'title',
        indicatorText
      )
