...
/* eslint-disable */
/*!
 * jQuery UI Droppable 1.9.0-beta.1
 *
...
;(function ($, undefined) {
  $.widget('ui.droppable', {
    ...
    _destroy() {
      ...
      for (let i = 0; i < drop.length; i++) if (drop[i] == this) drop.splice(i, 1)
  ...
  $.ui.intersect = function (draggable, droppable, toleranceMode) {
    ...
    const x1 = (draggable.positionAbs || draggable.position.absolute).left,
    ...
    drag(draggable, event) {
      ...
      $.each($.ui.ddmanager.droppables[draggable.options.scope] || [], function () {
        ...
        if (this.options.greedy) {
          const parent = this.element.parents(':data(droppable):eq(0)')
          if (parent.length) {
