...
import { IconDocumentLine as IconDocumentLineSvg, IconMsExcelLine as IconMsExcelLineSvg, IconMsPptLine as IconMsPptLineSvg, IconMsWordLine as IconMsWordLineSvg, IconPdfLine as IconPdfLineSvg, IconVideoLine as IconVideoLineSvg, IconAudioLine as IconAudioLineSvg } from '@instructure/ui-icons/es/svg';
import { isVideo, isAudio } from "../rce/plugins/shared/fileTypeUtils.js";
const stringIds = {
...
function onFileUploadInfoReceived(file, uploadInfo, onSuccess, onFailure) {
  const formData = createFormData(_objectSpread(_objectSpread({}, uploadInfo.upload_params), {}, {
    file
  }));
  ...
  axios.post(uploadInfo.upload_url, formData, config).then(response => onSuccess(response.data)).catch(response => onFailure(response));
}
