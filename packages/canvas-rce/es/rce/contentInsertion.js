...
import { renderImage, renderLinkedImage, renderVideo, renderAudio } from "./contentRendering.js";
import scroll from "../common/scroll.js";
import { cleanUrl, getAnchorElement, isOnlyTextSelected, isImageFigure } from "./contentInsertionUtils.js";
...
export function insertImage(editor, image) {
  ...
  if (shouldPreserveImgAnchor(editor)) {
    ...
    content = renderImage(_objectSpread({}, image));
  }
...
export function insertEquation(editor, latex, canvasUrl) {
  ...
  let url = `/equation_images/${encodeURIComponent(encodeURIComponent(latex))}?scale=${imgSz / docSz}`;
...
export function insertLink(editor, link) {
  const linkAttrs = _objectSpread({}, link);
...
function insertUndecoratedLink(editor, linkProps) {
  ...
  if (anchorElm && !editor.selection.isCollapsed()) {
    ...
    if (linkProps.userText && selectedPlainText !== linkText) {
      ...
      createLink(editor, selectedElm, void 0, linkAttrs);
    }
  } else {
    createLink(editor, selectedElm, linkText, linkAttrs);
  }
...
export function insertVideo(editor, video) {
  if (editor.selection.isCollapsed()) {
    ...
    let result = insertContent(editor, renderVideo(video)); // for some reason, editor.selection.getEnd() returned from
    ...
    return insertLink(editor, _objectSpread(_objectSpread({}, video), {}, {
      href: mediaPlayerURLFromFile(video)
    }));
  }
}
export function insertAudio(editor, audio) {
  if (editor.selection.isCollapsed()) {
    ...
    return insertLink(editor, _objectSpread(_objectSpread({}, audio), {}, {
      href: mediaPlayerURLFromFile(audio)
    }));
  }
}