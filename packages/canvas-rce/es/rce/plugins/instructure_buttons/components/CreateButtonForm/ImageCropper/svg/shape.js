...
import { SHAPE_CONTAINER_LENGTH } from "../constants.js";
...
function buildTriangle() {
  return createSvgElement('path', {
    d: `M${SHAPE_CONTAINER_LENGTH / 2} 0L${SHAPE_CONTAINER_LENGTH} ${SHAPE_CONTAINER_LENGTH}H0L${SHAPE_CONTAINER_LENGTH / 2} 0Z`
  });
}
...
function buildHexagon() {
  ...
  return createSvgElement('path', {
    d: `M${SHAPE_CONTAINER_LENGTH - leg} 0L${SHAPE_CONTAINER_LENGTH} ${SHAPE_CONTAINER_LENGTH / 2}L${SHAPE_CONTAINER_LENGTH - leg} ${SHAPE_CONTAINER_LENGTH}H${leg}L0 ${SHAPE_CONTAINER_LENGTH / 2}L${leg} 0H${SHAPE_CONTAINER_LENGTH - leg}Z`
  });
}
...
function buildOctagon() {
  ...
  return createSvgElement('path', {
    d: `M0 ${leg}L${leg} 0H${leg + side}L${SHAPE_CONTAINER_LENGTH} ${leg}V${leg + side}L${leg + side} ${SHAPE_CONTAINER_LENGTH}H${leg}L0 ${leg + side}V${leg}Z`
  });
}
