 ...
 */
import { BASE_SIZE, DEFAULT_OPTIONS, DEFAULT_SETTINGS, STROKE_WIDTH } from "./constants.js";
import { createSvgElement, convertFileToBase64 } from "./utils.js";
import { buildMetadata } from "./metadata.js";
import { buildShape } from "./shape.js";
import { buildText, buildTextBackground, getContainerWidth, getContainerHeight } from "./text.js";
export function buildSvg(settings, options = DEFAULT_OPTIONS) {
  settings = _objectSpread(_objectSpread({}, DEFAULT_SETTINGS), settings);
...
export function buildStylesheet() {
  return new Promise(resolve => resolve(fetch('/fonts/lato/extended/Lato-Bold.woff2'))).then(data => data.blob()).then(blob => convertFileToBase64(blob)).then(base64String => {
    const stylesheet = document.createElement('style');
    stylesheet.setAttribute('type', 'text/css');
    stylesheet.appendChild(document.createTextNode(`@font-face {font-family: "Lato Extended";font-weight: bold;src: url(${base64String});}`));
    return stylesheet;
  });
}
