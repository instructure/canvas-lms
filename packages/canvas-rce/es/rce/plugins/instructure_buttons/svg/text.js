 ...
 */
import { createSvgElement, splitTextIntoLines } from "./utils.js";
import { TEXT_BACKGROUND_PADDING, BASE_SIZE, TEXT_SIZE, MAX_CHAR_COUNT } from "./constants.js";
...
export function buildTextBackground({
  ...
  const initialX = Math.max(0, Math.floor(xValue - TEXT_BACKGROUND_PADDING)) + 4;
  const initialY = Math.floor(yValue - TEXT_SIZE[textSize] - TEXT_BACKGROUND_PADDING / 2);
  const horizontalLineLength = Math.floor(textWidth + paddingSize + 2) - 8;
  const verticalLineLength = Math.floor(textHeight + paddingSize + 2) - 8;
  pathElement.setAttribute('d', `M${initialX},${initialY} h${horizontalLineLength} a${4},${4} 0 0 1 ${4},${4} v${verticalLineLength} a${4},${4} 0 0 1 ${-4},${4} h${-horizontalLineLength} a${4},${4} 0 0 1 ${-4},${-4} v${-verticalLineLength} a${4},${4} 0 0 1 ${4},${-4} z`);
...
export function getContainerWidth({
  ...
  const textWidth = Math.floor(getTextWidth(text, textSize)) + TEXT_BACKGROUND_PADDING * 2 + 2;
