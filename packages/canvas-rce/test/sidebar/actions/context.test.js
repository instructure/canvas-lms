...
describe("Context actions", () => {
  it("change context type returns CHANGE_CONTEXT_TYPE with the new type", () => {
    ...
    assert.deepEqual(actions.changeContextType('new context'), {type: actions.CHANGE_CONTEXT_TYPE, payload: 'new context'})
  });

  it("change context id returns CHANGE_CONTEXT_ID with the new id", () => {

    assert.deepEqual(actions.changeContextId('19'), {type: actions.CHANGE_CONTEXT_ID, payload: '19'})
  });
  ...
  it("change context dispatches change context", () => {
    ...
    assert(dispatchSpy.calledWith({type: actions.CHANGE_CONTEXT, payload: {contextType: 'course', contextId: '27'}}))
  });
});