...
basicEditor = jest.fn((value = '') => {
    return {
        setValue: (newValue) => {
            value = newValue
            jest.spyOn(EquationEditorModal.prototype, 'getMathFiled').mockImplementation(jest.fn().mockReturnValue(value));
        },
        getValue: () => value,
...
describe('EquationEditorModal', () => {
    ...
    it('preserves content from advanced to initial field', () => {
        renderModal()
        toggleMode()
       let editor = editInAdvancedMode('hello')
        toggleMode()
        ...
        describe('correctly renders when', () => {
          ...
          `)
            ...
            it('deleting formula in basic editor', async () => {
                renderModal({openAdvanced: true})
                let editor=editInAdvancedMode('updated in advanced mode')
                toggleMode()
                await waitFor(() => {
