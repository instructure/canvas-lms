...
export default function ImageOptionsTray(props) {
  ...
  function handleSave(event) {
    ...
    if (urlHasError || altHasError || dimensionsHasError) {
      if (altHasError) { altRef?.current?.focus() }
      if (urlHasError) { urlRef?.current?.focus() }
      if (dimensionsHasError) { dimensionsRef?.current?.focus() }
      return
    }
            ...
            <Flex direction="column">
              {showUrlField && (
                <Flex.Item padding="small">
                  <UrlPanel fileUrl={url} urlRef={urlRef} setFileUrl={handleUrlChange} urlHasError={urlHasError} />
                </Flex.Item>
              )}
              <ImageOptionsForm
