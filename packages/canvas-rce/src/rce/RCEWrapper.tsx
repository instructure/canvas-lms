...
class RCEWrapper extends React.Component<RCEWrapperProps, RCEWrapperState> {
	...
	constructor(props: RCEWrapperProps) {
		...
		// @ts-expect-error
		this.initialContent = this.getRequiredFeatureStatuses()
		...
		this.state = {
			...
			shouldShowOnFocusButton:
				props.renderKBShortcutModal === undefined
					? true
					: props.renderKBShortcutModal,
	...
	insertEmbedCode(code: string) {
		...
		if (code_elem) {
			if (
				!code_elem.hasAttribute("title") &&
				!code_elem.hasAttribute("aria-label")
			) {
		...
		const element = contentInsertion.insertContent(editor, code);
		const ifr =
			element && element.querySelector && element.querySelector("iframe");
		if (ifr) {
	...
	insertImage(image: unknown) {
		const editor = this.mceInstance();
		const element = contentInsertion.insertImage(
			editor,
			image,
			this.getCanvasUrl(),
		);
	...
	insertVideo(video: unknown) {
		const editor = this.mceInstance();
		const element = contentInsertion.insertVideo(
			editor,
			video,
			this.getCanvasUrl(),
		);
	...
	insertAudio(audio: unknown) {
		const editor = this.mceInstance();
		const element = contentInsertion.insertAudio(
			editor,
			audio,
			this.getCanvasUrl(),
		);
	...
	insertLink(link: unknown) {
		const editor = this.mceInstance();
		const element = contentInsertion.insertLink(
			editor,
			link,
			this.getCanvasUrl(),
		);
	...
	getTextarea(): HTMLTextAreaElement | null {
		const node =
			this.props.textareaId && document.getElementById(this.props.textareaId);
	...
	getHtmlEditorStorage() {
		...
		const value =
			cookieValue || this.storage?.getItem?.("rce.htmleditor")?.content;
	...
	toggleView = (newView: string) => {
		...
		if (
			newView === PRETTY_HTML_EDITOR_VIEW ||
			newView === RAW_HTML_EDITOR_VIEW
		) {
	...
	_isFullscreen() {
		return !!(
			this.state.fullscreenState.isTinyFullscreen || document[FS_ELEMENT]
		);
	}
	...
	_enterFullscreen() {
		...
		if (typeof this._elementRef.current?.offsetHeight === "number") {
			this.setState({
				fullscreenState: {
					prevHeight:
						this._elementRef.current.offsetHeight - this._getStatusBarHeight(),
				},
			});
		}
		// @ts-expect-error
	...
	_handleFullscreenResize = () => {
		const ht =
			window.visualViewport?.height || document[FS_ELEMENT]?.offsetHeight;
	...
	is_dirty() {
		...
		const currentHtml = this.isHidden()
			? this.textareaValue()
			: this.mceInstance()?.getContent();
	...
	onInit = (_event: Event, editor: TinyMCEEditor) => {
		...
		editor.on(
			"Cut Change input Undo Redo",
			debounce(this.handleInputChange, 1000),
		);
		...
		if (this.iframe) {
			this.iframe.setAttribute(
				"title",
				formatMessage(
					"Rich Text Area. Press {OSKey}+F8 for Rich Content Editor shortcuts.",
					{
		...
		if (this.getRequiredFeatureStatuses().rce_find_replace) {
			editor.on("undo redo", (_e) => {
				if (
					editor?.dom?.doc?.getElementsByClassName?.("mce-match-marker")
						?.length > 0
				) {
	...
	_setupSelectionSaving = (editor) => {
		...
		const restoreSelectionIfNecessary = () => {
			...
			if (this.editor && savedSelection && selectionWasReset) {
				this.editor.selection.setRng(
					savedSelection.range,
					savedSelection.isForward,
				);
	...
	announceContextToolbars(editor: TinyMCEEditor) {
		editor.on("NodeChange", () => {
			...
			if (isImageEmbed(node, editor)) {
				if (this.announcing !== 1) {
					this.setState({
						announcement: formatMessage(
							"type Control F9 to access image options. {text}",
							{
				...
				if (this.announcing !== 2) {
					this.setState({
						announcement: formatMessage(
							"type Control F9 to access link options. {text}",
							{
				...
				if (this.announcing !== 3) {
					this.setState({
						announcement: formatMessage(
							"type Control F9 to access table options. {text}",
							{
								text: node.textContent,
							},
						),
		...
		editor.on("ResizeEditor", ({ deltaY }) => {
			...
			if (deltaY < 0) {
				this.setState({
					announcement: formatMessage(
						"The height of Rich Content Area is decreased.",
					),
				});
			} else {
				this.setState({
					announcement: formatMessage(
						"The height of Rich Content Area is increased.",
					),
				});
			}
		});
		...
		if (this.storage) {
			...
			try {
				...
				if (autosaved && autosaved.content) {
					...
					const editorContent = patchAutosavedContent(
						editor.getContent({ no_events: true }),
						true,
					);
					const autosavedContent = patchAutosavedContent(
						autosaved.content,
						true,
					);
	...
	cleanupAutoSave = (deleteAll = false) => {
		if (this.storage) {
			const expiry = deleteAll
				? Date.now()
				: Date.now() - (this.props.autosave?.maxAge || 0);
			let i = 0;
			let key;
	...
	onResize = (_e: unknown, coordinates: { deltaY: number }) => {
		...
		if (editor) {
			...
			if (!container) return;
			const currentContainerHeight = Number.parseInt(
	...
	KBShortcutModalExited = () => {
		if (this.state.KBShortcutFocusReturn === this.iframe) {
			...
			this.state.KBShortcutFocusReturn ===
			document.getElementById(`show-on-focus-btn-${this.id}`)
		) {
	...
	AIToolsExited = () => {
		if (this.state.AITToolsFocusReturn === this.iframe) {
			...
			this.state.AITToolsFocusReturn ===
			document.getElementById(`show-on-focus-btn-${this.id}`)
		) {
			// launched from showOnFocus button
			// edge case where focusing KBShortcutFocusReturn doesn't work
	...
	wrapOptions(options = {}) {
		const rcsExists = !!(
			this.props.trayProps?.host && this.props.trayProps?.jwt
		);
		...
		const wrappedOpts = {
			...
			menubar: mergeMenuItems(
				getMenubarForVariant(this.variant),
				possibleNewMenubarItems,
			),
			...
			plugins: mergePlugins(
				...
				// @ts-expect-error
				sanitizePlugins(options.plugins)?.filter(
					(p: string) => p.length > 0 && p[0] !== "-",
				),
		...
		if (this.state.shouldShowEditor) {
			if (!prevState.shouldShowEditor) {
				...
				if (prevProps.readOnly !== this.props.readOnly) {
					this.mceInstance().mode.set(
						this.props.readOnly ? "readonly" : "design",
					);
				}
			}
		}
	...
	resetAlertId = () => {
		if (this.state.messages.length > 0) {
			throw new Error(
				"There are messages currently, you cannot reset when they are non-zero",
			);
		}
		alertIdValue = 0;
	};
	...
	render() {
		...
		const statusBarFeatures = getStatusBarFeaturesForVariant(
			this.variant,
			statusBarOptions,
		);
		return (
						...
						return (
							...
							>
								{this.state.shouldShowOnFocusButton &&
									tinymce.Env.deviceType.isDesktop() && (
										<ShowOnFocusButton
								...
								/>
								{this.state.editorView === PRETTY_HTML_EDITOR_VIEW &&
								...
								{this.state.confirmAutoSave ? (
									<Suspense
										fallback={
											<Spinner renderTitle={renderLoading} size="small" />
										}
									>
