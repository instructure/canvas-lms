...
;(function() {
  ...
  $D.Translator = {
    ...
    finish(x) {
      ...
      if (
        ...
        this[this.unit + 's'] =
          this[this.unit + 's'] + (this.operator == 'add' ? 1 : -1) + (this.value || 0) * orient
  ...
  g.d = _.cache(
    _.process(
      _.each(_.rtoken(/^([0-2]\d|3[0-1]|\d)/), _.optional(g.ctoken2('ordinalSuffix'))),
      t.day
    )
  )
  g.dd = _.cache(
    _.process(_.each(_.rtoken(/^([0-2]\d|3[0-1])/), _.optional(g.ctoken2('ordinalSuffix'))), t.day)
  )
  ...
  g.MMM = g.MMMM = _.cache(
    _.process(g.ctoken('jan feb mar apr may jun jul aug sep oct nov dec'), t.month)
  )
  ...
  g.format = _.process(
    _.many(
      _.any(
        ...
        _.process(_.rtoken(/^(dd?d?d?|MM?M?M?|yy?y?y?|hh?|HH?|mm?|ss?|tt?|zz?z?|Z)/), function(
          fmt
        ) {
          if (g[fmt]) {
            return g[fmt]
