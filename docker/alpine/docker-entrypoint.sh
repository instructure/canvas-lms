...
if [[ "$(id -u)" -eq 0 && "${APP_USER}" != "root" && "${CANVAS_ALPINE_ENTRYPOINT_CHILD:-0}" != "1" ]]; then
  set_nofile_limit
  ensure_node_graceful_fs
  ensure_encryption_keys
  exec env \
    ...
    CANVAS_ALPINE_ENTRYPOINT_CHILD=1 \
    su-exec "${APP_USER}:${APP_USER}" "$0" "$@"
fi

set_nofile_limit
ensure_node_graceful_fs
ensure_encryption_keys
echo "Canvas Alpine entrypoint: soft nofile=$(ulimit -Sn 2>/dev/null || echo unknown) hard nofile=$(ulimit -Hn 2>/dev/null || echo unknown)"
