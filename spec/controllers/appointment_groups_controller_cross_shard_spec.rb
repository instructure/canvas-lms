...
describe AppointmentGroupsController, type: :request do
  ...
  describe "calendar event context codes" do
    it "returns consistent context codes for cross-shard appointment slots" do
      appointment = nil
      @shard2.activate do
      ...
      @shard1.activate do
        json = api_call_as_user(
          @user,
          :get,
          "/api/v1/appointment_groups?scope=reservable&include[]=appointments",
        ...
        ag_json = json.first
        expect(ag_json["appointments"]).to be_present

        first_appointment = ag_json["appointments"].first

        expect(first_appointment["effective_context_code"]).to be_present
        expect(first_appointment["all_context_codes"]).to be_present

        effective_context_code = first_appointment["effective_context_code"]
        all_context_codes = first_appointment["all_context_codes"]

        context_type, context_id = effective_context_code.split("_", 2)
        all_codes_array = all_context_codes.split(",").map(&:strip)

        matching_code = all_codes_array.find do |code|
          code_type, code_id = code.split("_", 2)
          next false unless code_type == context_type

          code == effective_context_code || code_id == context_id || code_id&.end_with?(context_id)
        end

        expect(matching_code).to be_present,
                                 "Expected all_context_codes '#{all_context_codes}' to include a matching code for '#{effective_context_code}'"
      end
    end
  end
