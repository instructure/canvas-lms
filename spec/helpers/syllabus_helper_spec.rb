  ...
  describe "#syllabus_user_content" do
    ...
    context "when context grants :read permission to current_user" do
      ...
      it "sends two arguments to `pulic_user_content`" do
        expect(helper).to receive(:user_content).with(@course.syllabus_body, context: @course, user: @teacher, is_public: false, location: "course_syllabus_#{@course.id}").once
    ...
    context "when context does not grant :read permission to current_user" do
      ...
      it "sends two arguments to `pulic_user_content`" do
        expect(helper).to receive(:user_content).with(@course.syllabus_body, context: @course, user: nil, is_public: true, location: "course_syllabus_#{@course.id}").once
        helper.syllabus_user_content
      end
    end
  end
