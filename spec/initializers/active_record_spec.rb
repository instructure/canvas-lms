      ...
      describe "with temp table" do
        it "should use a temp table when you select without an id" do
          ...
          User.select(:name).find_in_batches do |batch|
            User.connection.select_value("SELECT COUNT(*) FROM users_find_in_batches_temporary_table_#{User.select(:name).to_sql.hash.abs.to_s(36)}")
          end
        end
        ...
        it "should not use a temp table for a plain query" do
          ...
          User.find_in_batches do |batch|
            expect { User.connection.select_value("SELECT COUNT(*) FROM users_find_in_batches_temporary_table_#{User.scoped.to_sql.hash.abs.to_s(36)}") }.to raise_error
          end
        end
        ...
        it "should not use a temp table for a select with id" do
          ...
          User.select(:id).find_in_batches do |batch|
            expect { User.connection.select_value("SELECT COUNT(*) FROM users_find_in_batches_temporary_table_#{User.select(:id).to_sql.hash.abs.to_s(36)}") }.to raise_error
          end
        end
