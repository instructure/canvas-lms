...
QUnit.module('AssignmentColumnHeader: "Enter Grades as" Settings', function(hooks) {
  ...
  function getMenuItem(text) {
    ...
    return content.findWhere(
      component => component.name() === 'MenuItem' && component.text() === text
    )
  }
  ...
  function getMenuItemFlyout(text) {
    ...
    return content.findWhere(
      component => component.name() === 'MenuItemFlyout' && component.text().trim() === text
    )
  }

  function mountAndOpenMenu() {
    wrapper = mountAndOpenOptions(props)
    getMenuItemFlyout('Enter Grades as')
      .find('button')
      .simulate('mouseOver')
  }
  ...
  test('calls the onSelect callback with "points" when "Points" is selected', function() {
    let selected
    props.enterGradesAsSetting.selected = 'percent'
    props.enterGradesAsSetting.onSelect = value => {
      selected = value
    }
    mountAndOpenMenu()
...
test('the Curve Grades dialog has focus when it is invoked', function() {
  ...
  const allDialogCloseButtons = document.querySelectorAll(
...
test('disables the option when prop muteAssignmentAction.disabled is truthy', function() {
  ...
  equal(
    specificMenuItem.parentElement.parentElement.parentElement.getAttribute('aria-disabled'),
    'true'
  )
})
...
test('clicking the menu item invokes onSelect with correct callback', function() {
  ...
  equal(
    this.props.muteAssignmentAction.onSelect.getCall(0).args[0],
...
test('clicking the menu item invokes onSelect with correct callback', function() {
  ...
  equal(
    this.props.setDefaultGradeAction.onSelect.getCall(0).args[0],
    this.wrapper.instance().focusAtEnd
  )
})

test('the Set Default Grade dialog has focus when it is invoked', function() {
  const dialogManager = new SetDefaultGradeDialogManager(
...
test('clicking the menu item invokes onSelect with correct callback', function() {
  ...
  equal(
    this.props.downloadSubmissionsAction.onSelect.getCall(0).args[0],
    this.wrapper.instance().focusAtEnd
  )
})
...
test('clicking the menu item invokes the onSelect property with correct callback', function() {
  ...
  equal(
    this.props.reuploadSubmissionsAction.onSelect.getCall(0).args[0],
    this.wrapper.instance().focusAtEnd
  )
})
