...
QUnit.module('ViewOptionsMenu - notes', {
  ...
  getMenuItem() {
    ...
    return optionsMenu.findWhere(
      component => component.name() === 'MenuItem' && component.text().includes('Notes')
    )
  },
...
test('Module - First to Last is selected when criterion is module_position and direction is ascending', function() {
  ...
  const arrangeByMenu = new ReactWrapper(wrapper.node.arrangeByMenuContent, wrapper.node)
  const arrangeByMenuItems = arrangeByMenu.find('MenuItem').map(menuItem => menuItem)
  const selectedMenuItem = arrangeByMenuItems.find(
    menuItem => menuItem.text().trim() === 'Module - First to Last'
  )
...
test('Module - Last to First is selected when criterion is module_position and direction is ascending', function() {
  ...
  const arrangeByMenuItems = arrangeByMenu.find('MenuItem').map(menuItem => menuItem)
  const selectedMenuItem = arrangeByMenuItems.find(
    menuItem => menuItem.text().trim() === 'Module - Last to First'
  )
...
test('all column ordering options are disabled when the column ordering settings are disabled', function() {
  ...
  const arrangeByMenu = new ReactWrapper(wrapper.node.arrangeByMenuContent, wrapper.node)
  const disabledMenuItems = arrangeByMenu
    .find('MenuItem')
    .findWhere(menuItem => menuItem.props().disabled)
...
test('clicking on "Default Order" triggers onSortByDefault', function() {
  ...
  const arrangeByMenu = new ReactWrapper(wrapper.node.arrangeByMenuContent, wrapper.node)
  const arrangeByMenuItems = arrangeByMenu.find('MenuItem').map(menuItem => menuItem)
  const defaultOrderMenuItem = arrangeByMenuItems.find(
    menuItem => menuItem.props().children === 'Default Order'
  )
...
test('clicking on "Assignments - A-Z" triggers onSortByNameAscending', function() {
  ...
  const arrangeByMenu = new ReactWrapper(wrapper.node.arrangeByMenuContent, wrapper.node)
  const arrangeByMenuItems = arrangeByMenu.find('MenuItem').map(menuItem => menuItem)
  const assignmentNameAscendingMenuItem = arrangeByMenuItems.find(
    menuItem => menuItem.props().children === 'Assignment Name - A-Z'
  )
...
test('clicking on "Assignments - Z-A" triggers onSortByNameDescending', function() {
  ...
  const arrangeByMenu = new ReactWrapper(wrapper.node.arrangeByMenuContent, wrapper.node)
  const arrangeByMenuItems = arrangeByMenu.find('MenuItem').map(menuItem => menuItem)
  const assignmentNameDescendingMenuItem = arrangeByMenuItems.find(
    menuItem => menuItem.props().children === 'Assignment Name - Z-A'
  )
...
test('clicking on "Due Date - Oldest to Newest" triggers onSortByDueDateAscending', function() {
  ...
  const arrangeByMenu = new ReactWrapper(wrapper.node.arrangeByMenuContent, wrapper.node)
  const arrangeByMenuItems = arrangeByMenu.find('MenuItem').map(menuItem => menuItem)
  const dueDateOldestToNewestMenuItem = arrangeByMenuItems.find(
    menuItem => menuItem.props().children === 'Due Date - Oldest to Newest'
  )
...
test('clicking on "Due Date - Newest to Oldest" triggers onSortByDueDateDescending', function() {
  ...
  const arrangeByMenu = new ReactWrapper(wrapper.node.arrangeByMenuContent, wrapper.node)
  const arrangeByMenuItems = arrangeByMenu.find('MenuItem').map(menuItem => menuItem)
  const dueDateNewestToOldestMenuItem = arrangeByMenuItems.find(
    menuItem => menuItem.props().children === 'Due Date - Newest to Oldest'
  )
...
test('clicking on "Points - Lowest to Highest" triggers onSortByPointsAscending', function() {
  ...
  const arrangeByMenu = new ReactWrapper(wrapper.node.arrangeByMenuContent, wrapper.node)
  const arrangeByMenuItems = arrangeByMenu.find('MenuItem').map(menuItem => menuItem)
  const arrangeByPointsLowestToHighestMenuItem = arrangeByMenuItems.find(
    menuItem => menuItem.props().children === 'Points - Lowest to Highest'
  )
...
test('clicking on "Points - Highest to Lowest" triggers onSortByPointsDescending', function() {
  ...
  const arrangeByMenu = new ReactWrapper(wrapper.node.arrangeByMenuContent, wrapper.node)
  const arrangeByMenuItems = arrangeByMenu.find('MenuItem').map(menuItem => menuItem)
  const arrangeByPointsHighestToLowestMenuItem = arrangeByMenuItems.find(
    menuItem => menuItem.props().children === 'Points - Highest to Lowest'
  )
...
test('clicking Statuses calls onSelectShowStatusesModal', function() {
  ...
  const statusesMenuItem = optionsMenu.findWhere(
    component => component.name() === 'MenuItem' && component.text() === 'Statuses…'
  )
