...
QUnit.module('StatusesModal', function(suiteHooks) {
  ...
  test('after clicking apply in the color picker, the color picker popover is closed', function() {
    ...
    modalContent
      .find('StatusColorListItem PopoverTrigger Button')
      .at(0)
      .simulate('click')
    const colorPickerContent = new ReactWrapper(wrapper.node.colorPickerContents.late, wrapper.node)
    const applyButton = colorPickerContent
      .find('button')
      .findWhere(button => button.prop('children') === 'Apply')
    applyButton.simulate('click')
    strictEqual(
      modalContent
        .find('StatusColorListItem Popover')
        .at(0)
        .prop('show'),
      false
    )
  ...
  test('after clicking cancel in the color picker, the color picker popover is closed', function() {
    ...
    const applyButton = colorPickerContent
      .find('button')
      .findWhere(button => button.prop('children') === 'Cancel')
    applyButton.simulate('click')
    strictEqual(
      modalContent
        .find('StatusColorListItem Popover')
  ...
  test(
    ...
    function() {
      ...
      modalContent
        ...
        .at(0)
        .simulate('click')
      const colorPickerContent = new ReactWrapper(
        wrapper.node.colorPickerContents.late,
        wrapper.node
      )
      const whiteSwatch = colorPickerContent
        .find('button')
        .findWhere(button => button.prop('title') === 'white (#FFFFFF)')
