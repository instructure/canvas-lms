...
QUnit.module('Gradebook Data Loader', function(hooks) {
  ...
  test('includes all returned students when resolving gotStudents', function(assert) {
    ...
    respondWith(
      '/submissions',
      {student_ids: STUDENT_IDS.slice(0, 2)},
      200,
      {Link: ''},
      SUBMISSIONS_CHUNK_1
    )
    respondWith(
      '/submissions',
      {student_ids: STUDENT_IDS.slice(2, 3)},
      200,
      {Link: ''},
      SUBMISSIONS_CHUNK_2
    )
  ...
  test('requests submissions with pagination', function(assert) {
    ...
    respondWith(
      ...
      200,
      {},
      STUDENTS_PAGE_1.concat(STUDENTS_PAGE_2)
    )
    ...
    respondWith(
      '/submissions',
      {page: '2', student_ids: STUDENT_IDS},
      200,
      {Link: ''},
      SUBMISSIONS_CHUNK_2
    )
    ...
    const dataLoader = callLoadGradebookData({
      perPage: 50,
      submissionsChunkCb: saveSubmissions,
      submissionsChunkSize: 50
    })
    const resolve = assert.async()
  ...
  test('requests all paginated pages of custom columns', function(assert) {
    respondWith(
      ...
      {},
      200,
      {Link: '</customcols&page=2>; rel="last"'},
      CUSTOM_COLUMNS_PAGE_1
    )
  ...
  test("doesn't fetch custom column data until all other data is done", function(assert) {
    ...
    dataLoader.gotCustomColumns.then(() => {
      strictEqual(
        XHRS.filter(xhr => xhr.url.match(/data/)).length,
        0,
