...
test('brandVariableValue', function() {
  ...
  ok(c.brandVariableValue.calledWith(config, 'foo'), 'gets value for variable name')
...
test('startEditing: from scratch', function() {
  ...
  equal(
    window.sessionStorage.getItem('sharedBrandConfigBeingEdited'),
    null,
...
test('deteteSharedBrandConfig', function() {
  ...
  sandbox
    .mock(jQuery)
    .expects('ajaxJSON')
    .withArgs(`/api/v1/shared_brand_configs/${id}`, 'DELETE')
  const c = new CollectionView(props)
...
test('isActiveBrandConfig', function() {
  ...
  notOk(c.isActiveBrandConfig({md5: 'foo'}), 'false when there is no active brand config')
...
test('showsTooltipOnCopiedThemes', function() {
  ...
  notOk(oneConfigCollection.multipleThemesReflectActiveOne(), 'false if there is only one config')
  notOk(
...
test('isDeletable', function() {
  ...
  notOk(c.isDeletable(config), 'false if config has a different account id')

  config.account_id = props.accountID
  config.brand_config.md5 = props.activeBrandConfig.md5
  notOk(c.isDeletable(config), 'false if it is the active brand config')
