...
test('changes the "Export" menu item to indicate the export is in progress', function() {
  ...
  equal(
    this.menuItem.parentElement.parentElement.parentElement.getAttribute('aria-disabled'),
    'true'
  )
...
test('on success, shows the "New Export" menu item', function() {
  ...
  return this.wrapper
    ...
    .then(() => {
      ...
      const previousExportMenu = document.querySelector(
        '[role="menuitem"] [data-menu-id="previous-export"]'
      )
...
test('returns the previous export stored in the props if nothing is available in state', function() {
  ...
  const lastExportFromState = sandbox
    .stub(ActionMenu.prototype, 'lastExportFromState')
    .returns(undefined)
  const lastExportFromProps = sandbox
    .stub(ActionMenu.prototype, 'lastExportFromProps')
    .returns(previousExportProps().lastExport)
