define([
  ...
  test('submission has no tooltip for an unassigned student with no submission or ungraded submission', function() {
  ...
  test('submission has no tooltip for an unassigned student with a graded submission', function() {
    const override = createOverride({ type: 'student', id: OTHER_STUDENT_ID, dueAt: null });
  ...
  test('submission has no tooltip for an unassigned student with a graded submission (and last period is open)', function() {
    const override = createOverride({ type: 'student', id: OTHER_STUDENT_ID, dueAt: this.DATE_IN_OPEN_PERIOD });
    const assignment = createAssignment({ overrides: [override], gradedButNotAssigned: [student.id] });
    const map = createAndSetupMap(assignment, this.mapOptions);
    const state = map.getSubmissionState({ user_id: student.id, assignment_id: assignment.id });
  ...
  test('submission has no tooltip for an unassigned student with a graded submission (and last period is closed)', function() {
    const override = createOverride({ type: 'student', id: OTHER_STUDENT_ID, dueAt: this.DATE_IN_OPEN_PERIOD });
    const assignment = createAssignment({ overrides: [override], gradedButNotAssigned: [student.id] });
    this.mapOptions.gradingPeriods[2].isClosed = true;
    const map = createAndSetupMap(assignment, this.mapOptions);
    const state = map.getSubmissionState({ user_id: student.id, assignment_id: assignment.id });
    equal(state.tooltip, tooltipKeys.NONE);
  });
  ...
  test('submission has no tooltip for an assigned student with assignment due in a non-closed grading period', function() {
  ...
  test('submission has no tooltip for an assigned student with assignment due outside of any grading period', function() {
  ...
  test('submission has no tooltip for an assigned student with assignment with no due date', function() {
  ...
  test('submission shows "in closed period" toolip for an assigned student with overridden assignment due in a closed grading period', function() {
  ...
  test('user is admin: submission has no tooltip for an assigned student with overridden assignment due in a closed grading period', function() {
    const override = createOverride({ type: 'student', id: student.id, dueAt: this.DATE_IN_CLOSED_PERIOD });
    const assignment = createAssignment({ overrides: [override] });
    const mapOptions = Object.assign(this.mapOptions, { isAdmin: true })
    const map = createAndSetupMap(assignment, this.mapOptions);
  ...
  test('submission has no tooltip for an assigned student with overridden assignment due in a non-closed grading period', function() {
  ...
  test('submission has no tooltip for an assigned student with overridden assignment due outside of any grading period', function() {
  ...
  test('submission has no tooltip for an assigned student with overridden assignment with no due date', function() {
  ...
  test('submission shows "in closed period" tooltip for an assigned student with overridden assignment with multiple applicable overrides with the latest due date in a closed grading period', function() {
  ...
  test('user is admin: submission has no tooltip for an assigned student with overridden assignment with multiple applicable overrides with the latest due date in a closed grading period', function() {
    const studentOverride = createOverride({ type: 'student', id: student.id, dueAt: this.DATE_BEFORE_FIRST_PERIOD });
    const sectionOverride = createOverride({ type: 'section', id: student.sections[0], dueAt: this.DATE_IN_CLOSED_PERIOD });
    const groupOverride = createOverride({ type: 'group', id: student.group_ids[0], dueAt: this.DATE_IN_CLOSED_PERIOD });
    const assignment = createAssignment({ overrides: [studentOverride, sectionOverride, groupOverride] });
  ...
  test('submission has no tooltip for an assigned student with overridden assignment with multiple applicable overrides with the latest due date in a non-closed grading period', function() {
  ...
  test('submission has no tooltip for an assigned student with overridden assignment with multiple applicable overrides with the latest due date due outside of any grading period', function() {
    const studentOverride = createOverride({ type: 'student', id: student.id, dueAt: this.DATE_IN_CLOSED_PERIOD });
  ...
  test('submission has no tooltip for an assigned student with overridden assignment with with multiple applicable overrides with at least one override with no due date', function() {
    const studentOverride = createOverride({ type: 'student', id: student.id, dueAt: this.DATE_IN_CLOSED_PERIOD });
  ...
  test('submission shows "not in any period" tooltip for an unassigned student with a graded submission', function() {
    const override = createOverride({ type: 'student', id: OTHER_STUDENT_ID, dueAt: this.DATE_IN_OPEN_PERIOD });
    const assignment = createAssignment({ overrides: [override], gradedButNotAssigned: [student.id] });
    const map = createAndSetupMap(assignment, this.mapOptions);
    const state = map.getSubmissionState({ user_id: student.id, assignment_id: assignment.id });
  ...
  test('submission shows "in another period" tooltip for an assigned student with assignment with no due date', function() {
    const assignment = createAssignment({ dueAt: null });
    const map = createAndSetupMap(assignment, this.mapOptions);
    const state = map.getSubmissionState({ user_id: student.id, assignment_id: assignment.id });
    equal(state.tooltip, tooltipKeys.IN_ANOTHER_GP);
  ...
  test('submission shows "in another period" tooltip for an assigned student with overridden assignment due in a non-selected grading period', function() {
    const override = createOverride({ type: 'student', id: student.id, dueAt: this.DATE_IN_CLOSED_PERIOD });
    const assignment = createAssignment({ overrides: [override] });
    const map = createAndSetupMap(assignment, this.mapOptions);
    const state = map.getSubmissionState({ user_id: student.id, assignment_id: assignment.id });
  ...
  test('submission shows "not in any period" tooltip for an assigned student with overridden assignment due in no grading period', function() {
    const override = createOverride({ type: 'student', id: student.id, dueAt: this.DATE_AFTER_LAST_PERIOD });
    const assignment = createAssignment({ overrides: [override] });
    const map = createAndSetupMap(assignment, this.mapOptions);
    const state = map.getSubmissionState({ user_id: student.id, assignment_id: assignment.id });
  ...
  test('submission has no tooltip for an assigned student with overridden assignment due in the selected grading period', function() {
  ...
  test('submission shows "in another period" tooltip for an assigned student with overridden assignment with no due date', function() {
    const override = createOverride({ type: 'student', id: student.id, dueAt: null });
    const assignment = createAssignment({ overrides: [override] });
    const map = createAndSetupMap(assignment, this.mapOptions);
    const state = map.getSubmissionState({ user_id: student.id, assignment_id: assignment.id });
  ...
  test('submission shows "in another period" tooltip for an assigned student with overridden assignment with multiple applicable overrides with the latest due date in a non-selected grading period', function() {
    const studentOverride = createOverride({ type: 'student', id: student.id, dueAt: this.DATE_BEFORE_FIRST_PERIOD });
    const sectionOverride = createOverride({ type: 'section', id: student.sections[0], dueAt: this.DATE_BEFORE_FIRST_PERIOD });
    const groupOverride = createOverride({ type: 'group', id: student.group_ids[0], dueAt: this.DATE_IN_CLOSED_PERIOD });
    const assignment = createAssignment({ overrides: [studentOverride, sectionOverride, groupOverride] });
  ...
  test('submission shows "not in any period" tooltip for an assigned student with overridden assignment with multiple applicable overrides with the latest due date in no grading period', function() {
    const studentOverride = createOverride({ type: 'student', id: student.id, dueAt: this.DATE_BEFORE_FIRST_PERIOD });
    const sectionOverride = createOverride({ type: 'section', id: student.sections[0], dueAt: this.DATE_BEFORE_FIRST_PERIOD });
  ...
  test('submission has no tooltip for an assigned student with overridden assignment with multiple applicable overrides with the latest due date in the selected grading period', function() {
  ...
  test('submission shows "in another period" tooltip for an assigned student with overridden assignment with with multiple applicable overrides with at least one override with no due date', function() {
    const studentOverride = createOverride({ type: 'student', id: student.id, dueAt: this.DATE_IN_SELECTED_PERIOD });
    const sectionOverride = createOverride({ type: 'section', id: student.sections[0], dueAt: this.DATE_IN_CLOSED_PERIOD });
  ...
  test('submission shows "in another period" tooltip for an assigned student with overridden assignment due in no grading period', function() {
    const override = createOverride({ type: 'student', id: student.id, dueAt: this.DATE_AFTER_LAST_PERIOD });
    const assignment = createAssignment({ overrides: [override] });
    const map = createAndSetupMap(assignment, this.mapOptions);
    const state = map.getSubmissionState({ user_id: student.id, assignment_id: assignment.id });
  ...
  test('submission shows "in closed period" tooltip for an assigned student with overridden assignment due in the selected grading period', function() {
    const override = createOverride({ type: 'student', id: student.id, dueAt: this.DATE_IN_SELECTED_PERIOD });
    const assignment = createAssignment({ overrides: [override] });
    const map = createAndSetupMap(assignment, this.mapOptions);
    const state = map.getSubmissionState({ user_id: student.id, assignment_id: assignment.id });
  ...
  test('submission shows "in closed period" tooltip for an assigned student with overridden assignment with multiple applicable overrides with the latest due date in the selected grading period', function() {
    const studentOverride = createOverride({ type: 'student', id: student.id, dueAt: this.DATE_BEFORE_FIRST_PERIOD });
    const sectionOverride = createOverride({ type: 'section', id: student.sections[0], dueAt: this.DATE_BEFORE_FIRST_PERIOD });
  ...
  test('submission shows "in closed period" tooltip for an assigned student with assignment with no due date', function() {
    const assignment = createAssignment({ dueAt: null });
    const map = createAndSetupMap(assignment, this.mapOptions);
    const state = map.getSubmissionState({ user_id: student.id, assignment_id: assignment.id });
    equal(state.tooltip, tooltipKeys.IN_CLOSED_GP);
  ...
  test('submission shows "in closed period" tooltip for an assigned student with overridden assignment with no due date', function() {
    const override = createOverride({ type: 'student', id: student.id, dueAt: null });
    const assignment = createAssignment({ overrides: [override] });
    const map = createAndSetupMap(assignment, this.mapOptions);
    const state = map.getSubmissionState({ user_id: student.id, assignment_id: assignment.id });
  ...
  test('submission shows "in closed period" tooltip for an assigned student with overridden assignment with with multiple applicable overrides with at least one override with no due date', function() {
    const studentOverride = createOverride({ type: 'student', id: student.id, dueAt: null });
    const sectionOverride = createOverride({ type: 'section', id: student.sections[0], dueAt: this.DATE_AFTER_LAST_PERIOD });
    const groupOverride = createOverride({ type: 'group', id: student.group_ids[0], dueAt: this.DATE_IN_CLOSED_PERIOD });
    const assignment = createAssignment({ overrides: [studentOverride, sectionOverride, groupOverride] });
  ...
  test('submission shows no tooltip for an unassigned student with no submission or ungraded submission', function() {
    const override = createOverride({ type: 'student', id: OTHER_STUDENT_ID, dueAt: this.DATE_IN_OPEN_PERIOD });
    const assignment = createAssignment({ overrides: [override] });
    const map = createAndSetupMap(assignment, this.mapOptions);
    const state = map.getSubmissionState({ user_id: student.id, assignment_id: assignment.id });
  ...
  test('submission shows "not in any period" tooltip for an assigned student with overridden assignment with multiple applicable overrides with the latest due date in no grading period', function() {
    const studentOverride = createOverride({ type: 'student', id: student.id, dueAt: this.DATE_AFTER_LAST_PERIOD });
    const sectionOverride = createOverride({ type: 'section', id: student.sections[0], dueAt: this.DATE_IN_CLOSED_PERIOD });
    const groupOverride = createOverride({ type: 'group', id: student.group_ids[0], dueAt: this.DATE_IN_OPEN_PERIOD });
    const assignment = createAssignment({ overrides: [studentOverride, sectionOverride, groupOverride] });
    const map = createAndSetupMap(assignment, this.mapOptions);
