...
define(
  ['react', 'react-addons-test-utils', 'react-dom', 'jsx/collaborations/Collaboration', 'timezone'],
  (React, TestUtils, ReactDOM, Collaboration, tz) => {
    ...
    test('renders the date time in the correct format', () => {
      ...
      let component = TestUtils.renderIntoDocument(<Collaboration {...props} />)
      let dateString = ReactDOM.findDOMNode(
        TestUtils.findRenderedDOMComponentWithClass(component, 'DatetimeDisplay')
      ).innerText
    ...
    test('when the user clicks the trash button it opens the delete confirmation', () => {
      ...
      let component = TestUtils.renderIntoDocument(<Collaboration {...props} />)
      let trashIcon = ReactDOM.findDOMNode(
        TestUtils.findRenderedDOMComponentWithClass(component, 'icon-trash')
      )
      TestUtils.Simulate.click(trashIcon)
      let deleteConfirmation = TestUtils.findRenderedDOMComponentWithClass(
        component,
        'DeleteConfirmation'
      )
    ...
    test('when the user clicks the cancel button on the delete confirmation it removes the delete confirmation', () => {
      ...
      TestUtils.Simulate.click(trashIcon)
      let cancelButton = ReactDOM.findDOMNode(
        TestUtils.scryRenderedDOMComponentsWithClass(component, 'Button')[1]
      )
      TestUtils.Simulate.click(cancelButton)
      let deleteConfirmation = TestUtils.scryRenderedDOMComponentsWithClass(
        component,
        'DeleteConfirmation'
      )
