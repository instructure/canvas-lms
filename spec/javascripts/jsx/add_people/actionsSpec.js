...
define(
  ...
  (axios, {createStore}, api, {actions, actionTypes}, INITIAL_STATE) => {
    ...
    const failureData = {
      message: 'Error',
      response: {data: 'Error', status: 400, statusText: 'Bad Request', headers: {}}
    }
    const mockAxiosFail = () => {
    ...
    QUnit.module('Add People Actions', () => {
      ...
      test('dispatches VALIDATE_USERS_SUCCESS with data when successful', assert => {
        ...
        setTimeout(() => {
          ok(
            storeSpy.withArgs({type: actionTypes.VALIDATE_USERS_SUCCESS, payload: apiResponse})
              .calledOnce
          )
          resolved()
        }, 1)
      ...
      test('dispatches ENQUEUE_USERS_TO_BE_ENROLLED with data when validate users returns no dupes or missings', assert => {
        ...
        setTimeout(() => {
          ok(
            storeSpy.withArgs(sinon.match({type: actionTypes.ENQUEUE_USERS_TO_BE_ENROLLED}))
              .calledOnce
          )
          resolved()
        }, 1)
      ...
      test('dispatches VALIDATE_USERS_ERROR with error when fails', assert => {
        ...
        setTimeout(() => {
          ok(
            storeSpy.withArgs({type: actionTypes.VALIDATE_USERS_ERROR, payload: failureData})
              .calledOnce
          )
          resolved()
        }, 1)
      ...
      test('dispatches CREATE_USERS_SUCCESS with data when successful', assert => {
        ...
        runningState.userValidationResult.duplicates = {
          foo: {createNew: true, newUserInfo: newUser}
        }
        const resolved = assert.async()
        ...
        setTimeout(() => {
          ok(
            storeSpy.withArgs({type: actionTypes.CREATE_USERS_SUCCESS, payload: apiResponse})
              .calledOnce
          )
          ok(
            storeSpy.withArgs({type: actionTypes.ENQUEUE_USERS_TO_BE_ENROLLED, payload: [newUser]})
          )
          resolved()
        }, 1)
      })
      ...
      test('dispatches CREATE_USERS_ERROR with error when fails', assert => {
        ...
        setTimeout(() => {
          ok(
            storeSpy.withArgs({type: actionTypes.CREATE_USERS_ERROR, payload: failureData})
              .calledOnce
          )
          resolved()
        }, 1)
      ...
      test('dispatches SUCCESS with data when successful', assert => {
        ...
        setTimeout(() => {
          ok(
            storeSpy.withArgs({type: actionTypes.ENROLL_USERS_SUCCESS, payload: {data: 'foo'}})
              .calledOnce
          )
          resolved()
        }, 1)
      ...
      test('dispatches ERROR with error when fails', assert => {
        ...
        setTimeout(() => {
          ok(
            storeSpy.withArgs({type: actionTypes.ENROLL_USERS_ERROR, payload: failureData})
              .calledOnce
          )
          resolved()
        }, 1)
      ...
      test('dispatches dependent actions', (/* assert */) => {
        ...
        runningState.userValidationResult.duplicates = {
          foo: {selectedUserId: 1, newUserInfo: {email: 'foo', name: 'bar'}}
        }
      ...
      test('dispatches dependent actions', (/* assert */) => {
        ...
        ok(
          storeSpy.withArgs({type: actionTypes.SKIP_DUPLICATE, payload: {address: 'foo'}})
            .calledOnce,
          'SKIP_DUPLICATE'
        )
      })
