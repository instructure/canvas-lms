...
define([
  ...
  const rawMissingList = [
    {address: 'amelia', type: 'unique_id'}, {address: 'dbcooper', type: 'unique_id'}
  ];
  ...
  const goodUsers = [
    {address: 'john', user_name: 'John Lennon', account_id: 8, account_name: 'British Invasion', user_id: 7},
    {address: 'paul', user_name: 'Paul McCartney', account_id: 8, account_name: 'British Invasion', user_id: 8},
    {address: 'georg', user_name: 'George Harrison', account_id: 8, account_name: 'British Invasion', user_id: 9},
    {address: 'ringo', user_name: 'Ringo Starr', account_id: 8, account_name: 'British Invasion', user_id: 10},
  ]
  ...
  const API_CREATE_RESPONSE2 = {
    ...
    errored_users: [{
      ...
      existing_users: [{address: 'who@cares.net', user_id: 12, user_name: 'Who Cares', account_id: 1, account_name: 'TV Land'}]
    }]
  };
  ...
  test('set input paramaters', () => {
    ...
    deepEqual(newState.userValidationResult, INITIAL_STATE.userValidationResult, 'userValidationResult');
  ...
  test('VALIDATE_USERS_ERROR', () => {
    ...
    deepEqual(newState.courseParams, INITIAL_STATE.courseParams, 'courseParams');
    deepEqual(newState.inputParams, INITIAL_STATE.inputParams, 'inputParams');
    deepEqual(newState.apiState, {pendingCount: 0, error: 'whoops'}, 'api is no longer in-flight with error');
    deepEqual(newState.userValidationResult, INITIAL_STATE.userValidationResult, 'userValidationResult');
    deepEqual(newState.usersToBeEnrolled, INITIAL_STATE.usersToBeEnrolled, 'usersToBeEnrolled');
    equal(newState.usersEnrolled, INITIAL_STATE.usersEnrolled, 'usersEnrolled');
  });
  ...
  test('CHOOSE_DUPLICATE', () => {
    ...
    equal(newState.userValidationResult.duplicates.bob.selectedUserId, 2, 'duplicate set selectedUserId');
  ...
  test('SKIP_DUPLICATE', () => {
    ...
    equal(newState.userValidationResult.duplicates.bob.createNew, false, 'duplicate set createNew');
    equal(newState.userValidationResult.duplicates.bob.selectedUserId, -1, 'duplicate set selectedUserId');
  ...
  test('ENQUEUE_NEW_FOR_DUPLICATE', () => {
    // create a new bob
    const newState = reduce(actions.enqueueNewForDuplicate({address: 'bob', newUserInfo: USER_BOB}), runningState);
    deepEqual(newState.courseParams, INITIAL_STATE.courseParams, 'courseParams');
    deepEqual(newState.inputParams, INITIAL_STATE.inputParams, 'inputParams');
    deepEqual(newState.apiState, INITIAL_STATE.apiState, 'apiState');
    ...
    equal(newState.userValidationResult.duplicates.bob.selectedUserId, -1, 'duplicate set selectedUserId');
    equal(newState.userValidationResult.duplicates.bob.skip, false, 'duplicate set skip');
    deepEqual(newState.userValidationResult.duplicates.bob.newUserInfo, USER_BOB, 'duplicate set newUserInfo');
  ...
  test('ENQUEUE_NEW_FOR_MISSING', () => {
    ...
    const newState = reduce(actions.enqueueNewForMissing({address: 'amelia', newUserInfo: USER_AMELIA}), runningState);
    ...
    deepEqual(newState.userValidationResult.missing.amelia.newUserInfo, USER_AMELIA, 'missing entry has newUserInfo');
  ...
  test('ENQUEUE_NEW_FOR_MISSING, unselect user', () => {
    const newState = reduce(actions.enqueueNewForMissing({address: 'amelia', newUserInfo: false}), runningState);
    deepEqual(newState.courseParams, INITIAL_STATE.courseParams, 'courseParams');
    deepEqual(newState.inputParams, INITIAL_STATE.inputParams, 'inputParams');
    deepEqual(newState.apiState, INITIAL_STATE.apiState, 'apiState');

    equal(newState.userValidationResult.missing.amelia.createNew, false, 'missing entry unset createNew');
    deepEqual(newState.userValidationResult.missing.amelia.newUserInfo, USER_AMELIA, 'missing entry still has newUserInfo');
  ...
  test('ENROLL_USERS_ERROR', () => {
    ...
    deepEqual(newState.courseParams, runningState.courseParams, 'courseParams');
    deepEqual(newState.inputParams, runningState.inputParams, 'inputParams');
    deepEqual(newState.apiState, {pendingCount: 0, error: 'uh oh'}, 'api is no longer in-flight and has error');
    deepEqual(newState.userValidationResult, runningState.userValidationResult, 'userValidationResult');
    deepEqual(newState.usersToBeEnrolled, runningState.usersToBeEnrolled, 'usersToBeEnrolled');
    equal(newState.usersEnrolled, runningState.usersEnrolled, 'usersEnrolled');
  });
