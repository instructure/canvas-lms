...
test('uploadFile requests no_redirect in preflight even if not specified', () => {
  ...
  return uploadFile(url, data, file, fakeAjaxLib).then(() => {
    ok(postStub.calledWith(url, {name: 'fake', no_redirect: true}), 'posted with no_redirect: true')
  })
})
...
test('uploadFile differentiates network failures during preflight', () => {
  ...
  return uploadFile('http://preflightUrl', {}, file, fakeAjaxLib)
    ...
    .catch(({message}) =>
      ok(message.match(/failed to initiate the upload/), 'correct error message')
    )
})
...
test('uploadFile differentiates network failures after upload', () => {
  ...
  return uploadFile('http://preflightUrl', {}, file, fakeAjaxLib)
    ...
    .catch(({message}) =>
      ok(message.match(/failed to complete the upload/), 'correct error message')
    )
})
