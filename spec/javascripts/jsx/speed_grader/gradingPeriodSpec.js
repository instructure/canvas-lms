...
require(['jsx/speed_grader/gradingPeriod'], ({
  ...
  const gradingPeriods = [closedGradingPeriod, pastGradingPeriod, currentGradingPeriod]
  ...
  test('gradingPeriodForDate', () => {
    ...
    ok(gradingPeriodForDate(currentGPDate, gradingPeriods) === currentGradingPeriod)

    const invalidDate = new Date(NOW + MS_IN_DAY * 10).toISOString()
    ok(gradingPeriodForDate(invalidDate, gradingPeriods) === undefined)
  ...
  test('dueDateForStudent', () => {
    ok(
      dueDateForStudent(student, [{default: true, due_at: NOW}]) === NOW,
      'no assignment overrides returns default due date'
    )

    ok(
      dueDateForStudent(student, [
    ...
    ok(
      dueDateForStudent({...student, section_ids: [2]}, [
    ...
    ok(
      dueDateForStudent(student, [{default: true, due_at: null}, studentOverride]) == null,
  ...
  test('assignmentClosedForStudent', () => {
    ...
    ok(
      !assignmentClosedForStudent(student, {
        due_at: dueAt,
    ...
    ok(
      assignmentClosedForStudent(closedStudent, {
        due_at: dueAt,
        gradingPeriods: gradingPeriods,
        assignmentOverrides: overrides
      })
