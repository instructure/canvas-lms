...
define(
  ['react', 'react-addons-test-utils', 'jquery', 'jsx/files/Toolbar'],
  (React, TestUtils, $, Toolbar) => {
    ...
    test('renders multi select action items when there is more than one item selected', () => {
      ...
      ok(
        $(toolbar.getDOMNode()).find('.ui-buttonset .ui-button').length,
        'shows multiple select action items'
      )
    })
    ...
    test('renders only view and download buttons for limited users', () => {
      ...
      ok(
        buttonsEnabled($(toolbar.getDOMNode()), config),
        'only view and download buttons are shown'
      )
    })
