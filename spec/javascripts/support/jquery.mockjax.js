...
define(['jquery'], ($) => {
  ...
  function parseXML (xml) {
    ...
    try {
      ...
      const msg = (e.name == undefined ? e : `${e.name}: ${e.message}`);
  ...
  $.extend({
    ajax (url, origSettings) {
      ...
      let s = jQuery.extend(true, {}, jQuery.ajaxSettings, origSettings),
			    mock = false;
			// Iterate over our mock handlers (in registration order) until we find
			// one that is willing to intercept the request
      $.each(mockHandlers, (k, v) => {
        if (!mockHandlers[k]) {
          return;
        }
        let m = null;
        ...
        if (m) {
          ...
          const c = $.extend({}, $.mockjaxSettings, m);
          ...
          if (s.dataType === 'script' && s.type.toUpperCase() === 'GET' && remote) {
            ...
            const callbackContext = origSettings && origSettings.context || s;
      ...
      if (!mock) {
        return _ajax.call($, url, origSettings);
      }
      return mock;
    }
  });
