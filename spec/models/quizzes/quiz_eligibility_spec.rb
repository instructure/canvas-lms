...
describe Quizzes::QuizEligibility do
  ...
  describe "#eligible?" do
    ...
    describe "date-based overrides" do
      ...
      let(:active_course)     { Course.new(end_at: Time.zone.now + 3.days) }
      let(:concluded_course)  { Course.new(end_at: Time.zone.now - 3.days) }

      let(:active_section)    { CourseSection.new(end_at: Time.zone.now + 3.days, restrict_enrollments_to_section_dates: true) }
      let(:concluded_section) { CourseSection.new(end_at: Time.zone.now - 3.days, restrict_enrollments_to_section_dates: true) }
      ...
      context "term concluded" do
        ...
        it "returns true if course overrides" do
          eligibility.stubs(:term).returns(concluded_term)
        ...
        it "returns true if section overrides" do
          eligibility.stubs(:term).returns(concluded_term)
          eligibility.stubs(:course).returns(concluded_course)
          eligibility.stubs(:course_section).returns(active_section)
          expect(eligibility.eligible?).to be_truthy
