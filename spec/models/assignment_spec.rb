...
describe Assignment do
  ...
  describe ".preload_hidden_submissions_count" do
    before(:once) do
      PostPolicy.enable_feature!
    ...
    it "preloads hidden submission counts on each assignment passed in" do
      assignment2 = @course.assignments.create!(title: "Assignment 2")
      Assignment.preload_hidden_submissions_count([@assignment, assignment2])
      # This assertion checks to make sure the counts are preloaded, because
      # if they weren't preloaded, Assignment#hidden_submissions_count would
      # invoke Assignment.preload_hidden_submissions_count
      expect(Assignment).not_to receive(:preload_hidden_submissions_count)
      @assignment.hidden_submissions_count
      assignment2.hidden_submissions_count
    end

    it "includes submissions that are graded and not posted in the count" do
      @assignment.grade_student(@student, grader: @teacher, score: 5)
      Assignment.preload_hidden_submissions_count([@assignment])
      expect(@assignment.hidden_submissions_count).to eq 1
    end

    it "excludes submissions that are not graded from the count" do
      Assignment.preload_hidden_submissions_count([@assignment])
      expect(@assignment.hidden_submissions_count).to be_zero
    end

    it "excludes submissions that are posted from the count" do
      @assignment.grade_student(@student, grader: @teacher, score: 5)
  ...
  describe "#hidden_submissions_count" do
    ...
    it "preloads the submission count if it isn't already loaded" do
      expect(Assignment).to receive(:preload_hidden_submissions_count).once.and_call_original
      @assignment.hidden_submissions_count
    end

    it "does not preload the submission count if it is already loaded" do
      Assignment.preload_hidden_submissions_count([@assignment])
      expect(Assignment).not_to receive(:preload_hidden_submissions_count)
      @assignment.hidden_submissions_count
    end
    ...
    it "excludes submissions that are posted from the count" do
      @assignment.grade_student(@student, grader: @teacher, score: 5)
      @assignment.submissions.find_by(user: @student).update!(posted_at: Time.zone.now)
      expect(@assignment.hidden_submissions_count).to be_zero
    end
  end
