...
describe ConditionalRelease::Service do
  before :once do
    CRService = ConditionalRelease::Service
  end

  def enable_service
    allow(CRService).to receive(:enabled_in_context?).and_return(true)
  end
  ...
  describe 'env_for' do
    before do
      enable_service
      allow(CRService).to receive(:active_rules).and_return([])
      course_with_student(active_all: true)
    end

    it 'returns no env if not enabled' do
      allow(CRService).to receive(:enabled_in_context?).and_return(false)
      env = CRService.env_for(@course, @student)
      expect(env).not_to have_key :CONDITIONAL_RELEASE_ENV
    end

    it 'returns no env if user not specified' do
      env = CRService.env_for(@course)
      expect(env).not_to have_key :CONDITIONAL_RELEASE_ENV
    end
    ...
    it 'includes a relevant rule if includes :rule' do
      assignment_model course: @course
      allow(CRService).to receive(:rule_triggered_by).and_return(nil)
      env = CRService.env_for(@course, @student, assignment: @assignment, includes: [:rule])
      cr_env = env[:CONDITIONAL_RELEASE_ENV]
    ...
    it 'includes a active rules if includes :active_rules' do
      assignment_model course: @course
      allow(CRService).to receive(:rule_triggered_by).and_return(nil)
      env = CRService.env_for(@course, @student, assignment: @assignment, includes: [:active_rules])
      cr_env = env[:CONDITIONAL_RELEASE_ENV]
