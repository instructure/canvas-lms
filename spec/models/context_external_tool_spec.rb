...
describe ContextExternalTool do
  ...
  describe "logo_for" do
    before :each do
      @tool = @root_account.context_external_tools.new(
        :name => 'tool',
        :consumer_key => '12345',
    ...
    context "when the tool is not selected" do
      context "and nothing is configured" do
        it "should return nil" do
          @tool.save!
          expect(@tool.logo_for(:global_navigation)).to be_nil
        end
      end

      context "and there is an icon_url at the root, but none for the given type" do
        it "should return the tool's icon_url" do
          @tool.settings = {:icon_url => 'logo.png', :global_navigation => {:text => 'Clickbait'}}
          @tool.save!
          expect(@tool.logo_for(:global_navigation)).to eq('logo.png')
        end
      end

      context "and there is an icon_url configured for the given type" do
        it "should return the configured icon_url" do
          @tool.settings = {:icon_url => 'default-logo.png', :global_navigation => {:icon_url => 'logo.png'}}
          @tool.save!
          expect(@tool.logo_for(:global_navigation)).to eq('logo.png')
        end
      end
    ...
    context "when the tool is selected" do
      context "and nothing is configured" do
        it "should return nil" do
          @tool.save!
          expect(@tool.logo_for(:global_navigation, true)).to be_nil
        end
      end

      context "and there is no icon_selected_url configured, but there's an icon_url at the root" do
        it "should return the tool's icon_url" do
          @tool.settings = {:icon_url => 'logo.png', :global_navigation => {:text => 'Clickbait'}}
          @tool.save!
          expect(@tool.logo_for(:global_navigation, true)).to eq('logo.png')
        end
      end

      context "and there is no icon_selected_url configured, but there's an icon_url configured for the given type" do
        it "should return the configured icon_url" do
          @tool.settings = {:icon_url => 'default-logo.png', :global_navigation => {:icon_url => 'logo.png'}}
          @tool.save!
          expect(@tool.logo_for(:global_navigation, true)).to eq('logo.png')
      ...
      context "and there is an icon_selected_url at the root, but none for the given type" do
        it "should return the tool's icon_selected_url" do
          @tool.settings = {
            :icon_url => 'logo.png',
            :icon_selected_url => 'logo-selected.png',
            :global_navigation => {:text => 'Clickbait'}
          }
          @tool.save!
          expect(@tool.logo_for(:global_navigation, true)).to eq('logo-selected.png')
        end
      end

      context "and there is an icon_selected_url configured for the given type" do
        it "should return the configured icon_selected_url" do
          @tool.settings = {:global_navigation => {:icon_url => 'logo.png', :icon_selected_url => 'logo-selected.png'}}
          @tool.save!
          expect(@tool.logo_for(:global_navigation, true)).to eq('logo-selected.png')
        end
      end
