...
describe ContentMigration do
  context "blueprint settings" do
    ...
    it "does not copy blueprint settings if not requested" do
      run_course_copy
      expect(MasterCourses::MasterTemplate.is_master_course?(@copy_to)).to eq false
    end
    ...
    it "copies blueprint settings" do
      ...
      expect(template_to.use_default_restrictions_by_type).to eq true
      ...
      expect(mt.use_default_restrictions).to eq true
      ...
      expect(mt.use_default_restrictions).to eq false
    ...
    it "does nothing if the destination course is already associated with a blueprint course" do
      ...
      expect(MasterCourses::MasterTemplate.is_master_course?(@copy_to)).to eq false
      expect(MasterCourses::ChildSubscription.is_child_course?(@copy_to)).to eq true
    end
