...
describe "profile/_ways_to_contact" do
  ...
  describe "MFA settings and SMS registration" do
    before do
    ...
    context "when MFA is disabled and SMS is enabled in US region" do
      let(:mfa_settings) { :disabled }
      let(:otp_via_sms) { true }

      it "does not render SMS registration link" do
        course_with_student
        view_context
        assign(:user_data, { can_edit_channels: true })
        assign(:email_channels, [])
        ...
        render partial: "profile/ways_to_contact"
        expect(response).not_to be_nil
        expect(response.body).not_to match(/register_sms_number/)
      end
    end

    context "when MFA is optional and SMS is enabled in US region" do
      let(:mfa_settings) { :optional }
      let(:otp_via_sms) { true }

      it "renders SMS registration link" do
        course_with_student
        view_context
        assign(:user_data, { can_edit_channels: true })
        assign(:email_channels, [])
        ...
        render partial: "profile/ways_to_contact"
        expect(response).not_to be_nil
        expect(response.body).to match(/register_sms_number/)
      end
    end

    context "when MFA is required for admins and SMS is enabled in US region" do
      let(:mfa_settings) { :required_for_admins }
      let(:otp_via_sms) { true }
    ...
    context "when MFA is required and SMS is enabled in US region" do
    ...
    context "when MFA is required and SMS is disabled in US region" do
      let(:mfa_settings) { :required }
