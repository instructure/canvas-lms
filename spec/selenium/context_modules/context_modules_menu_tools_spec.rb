...
describe "context modules" do
  ...
  context "module index tool placement" do
    ...
    context "individual module menu tool with external apps FF enabled" do
      before(:once) do
        Account.site_admin.enable_feature!(:create_external_apps_side_tray_overrides)
      end

      shared_examples "opens the external apps modal successfully" do
        it "opens the external apps modal when triggered from the module menu", custom_timeout: 60 do
          visit_modules_index_page(@course.id)
          manage_module_button(@module2).click
          expect(module_settings_menu(@module2.id)).to include_text("External Apps...")
          module_index_menu_tool_link("External Apps...").click
          wait_for_ajaximations
    ...
    context "individual module menu tool with external apps FF disabled" do
      before(:once) do
  ...
  context "menu tools", priority: "1" do
    ...
    context "displays external apps in popover modal" do
      before(:once) do
        Account.site_admin.disable_feature!(:create_external_apps_side_tray_overrides)
      end
