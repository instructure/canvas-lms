...
describe "better_file_browsing, folders" do
  ...
  context "Folders" do
    ...
    it "displays the new folder form", priority: "1", test_id: 268052 do
      click_new_folder_button
      expect(f("form.ef-edit-name-form")).to be_displayed
    end

    it "creates a new folder", :xbrowser, priority: "1", test_id: 133121 do
    ...
    it "displays all cog icon options", priority: "1", test_id: 133124 do
      expect(fj("a:contains('new test folder')")).to be_present
    ...
    it "edits folder name", priority: "1", test_id: 223501 do
    ...
    it "validates xss on folder text", priority: "1", test_id: 133113 do
      add_folder('<script>alert("Hi");</script>')
      expect(ff('.ef-name-col__text')[0].text).to eq '<script>alert("Hi");<_script>'
    end

    it "moves a folder", priority: "1", test_id: 133125 do
    ...
    it "deletes a folder from cog icon", priority: "1", test_id: 223502 do
      skip_if_safari(:alert)
      delete(0, :cog_icon)
      expect(f("#content")).not_to contain_link("new test folder")
    end

    it "unpublishes and publish a folder from cloud icon", priority: "1", test_id: 220354 do
    ...
    it "makes folder available to student with link", priority: "1", test_id: 133110 do
    ...
    it "makes folder available to student within given timeframe", priority: "1", test_id: 193160 do
    ...
    it "deletes folder from toolbar", priority: "1", test_id: 133105 do
      skip_if_safari(:alert)
      delete(0, :toolbar_menu)
      expect(f("body")).not_to contain_css('.ef-item-row')
    end

    it "is able to create and view a new folder with uri characters", priority: "2", test_id: 193153 do
  ...
  context "Folder Tree" do
    ...
    it "creates 15 new child folders and show them in the FolderTree when expanded", priority: "2", test_id: 121886 do
      create_new_folder
