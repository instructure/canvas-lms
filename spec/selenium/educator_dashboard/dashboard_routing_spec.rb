...
describe "educator dashboard routing", :ignore_js_errors do
  ...
  def educator_dashboard_displayed?
    element_exists?("body.educator-dashboard")
  end
  ...
  describe "rendering" do
    context "does not show" do
      it "does not render for student-only user" do
      ...
      it "does not render for observer" do
      ...
      it "does not render for K5 teacher" do
      ...
      it "does not render for teacher with only completed enrollments" do
        teacher = user_factory(active_all: true)
    ...
    context "shows" do
      it "renders for active teacher" do
        user_session(@teacher)
        get "/"
        wait_for_ajaximations
      ...
      it "renders for invited teacher" do
        teacher = user_factory(active_all: true)
        enrollment = @course.enroll_teacher(teacher, enrollment_state: :active)
      ...
      it "renders for teacher who also has a student enrollment" do
        teacher = user_factory(active_all: true)
        @course.enroll_teacher(teacher, enrollment_state: :active)
