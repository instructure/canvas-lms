...
module WikiAndTinyCommon
  ...
  def type_tiny_list(number_list_items)
    (1..number_list_items).each do |i|
      type_in_tiny('textarea.body', "#{i}")
      next if i == number_list_items
      in_frame wiki_page_body_ifr_id do
        f('#tinymce').send_keys(:return)
      end
    end
  ...
  def create_formatted_list(number_list_items, type_of_list)
    if type_of_list == "ordered"
      list_tag = "ol"
    elsif type_of_list == "unordered"
      list_tag = "ul"
    else
      list_tag = nil
    end
    compare_string = "<#{list_tag}>\r\n"
    (1..number_list_items).each do |i|
      compare_string += "<li>#{i}</li>\r\n"
    end
    compare_string += "</#{list_tag}>"
  end

  def create_unformatted_list(number_list_items)
    compare_string = "<p>"
    (1..number_list_items).each do |i|
      compare_string += "#{i}"
      next if i == number_list_items
