...
describe "quizzes question creation with attempts" do
  ...
  context 'quiz attempts' do
    ...
    it "does not allow quiz attempts that are entered with letters", priority: '2', test_id: 206029 do
      skip('fragile')
      fill_out_attempts_and_validate('abc', 'Quiz attempts can only be specified in numbers', '')
    end

    it "does not allow quiz attempts that are more than 3 digits long", priority: '2', test_id: 206030 do
      skip('fragile')
    ...
    it "does not allow quiz attempts that are letters and numbers mixed", priority: '2', test_id: 206036 do
      skip('fragile')
      fill_out_attempts_and_validate('31das', 'Quiz attempts can only be specified in numbers', '')
    end

    it "allows a 3 digit number for a quiz attempt", priority: '2', test_id: 206039 do
      attempts = "123"
      click_settings_tab
