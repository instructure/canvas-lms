...
describe "viewing a quiz with variable due dates on the quizzes index page" do
  ...
  context "as a student in Section A" do
    ...
    it "shows the availability dates for Section A", priority: "1" do
      date_available = f(".date-available")
      expect(date_available).to include_text("Available until")
      expect(date_available).to include_text(format_date_for_view(@lock_at_a, :short).to_s)
    end
  end
  ...
  context "as a student in Section B" do
    ...
    it "shows the availability dates for Section B", priority: "1" do
      date_available = f(".date-available")
      expect(date_available).to include_text("Not available until")
      expect(date_available).to include_text(format_date_for_view(@unlock_at_b, :short).to_s)
    end
  end
end
