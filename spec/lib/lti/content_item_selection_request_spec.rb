...
describe Lti::ContentItemSelectionRequest do
  ...
  describe "#generate_lti_launch" do
    ...
    context "params" do
      ...
      it "adds the 'ext_lti_assignment_id' if available" do
        ...
        lti_request = described_class.new(**default_params, secure_params:)
        lti_launch = lti_request.generate_lti_launch(placement:)
      ...
      context "return_url" do
        ...
        it "generates a url a http protocol when the base_uri uses http" do
          ...
          lti_request_with_scheme = described_class.new(**default_params, base_url: base_uri.to_s)
          create_url = Rails.application.routes.url_helpers.course_external_content_success_url(
            host: base_uri.host,
            protocol: base_uri.scheme,
            course_id: course.id,
        ...
        it "generates a url with a port when there is a port in the base_uri" do
          base_uri.port = 8080
          lti_request_with_port = described_class.new(**default_params, base_url: base_uri.to_s)
          create_url = Rails.application.routes.url_helpers.course_external_content_success_url(
            host: base_uri.host,
            protocol: base_uri.scheme,
            port: base_uri.port,
  ...
  describe ".default_lti_params" do
    before do
      allow(Lti::V1p1::Asset).to receive(:opaque_identifier_for).with(course).and_return("course_opaque_id")
    end
    ...
    it "adds user information when a user is provided" do
      allow(Lti::V1p1::Asset).to receive(:opaque_identifier_for).with(teacher, context: course).and_return("teacher_opaque_id")
