...
describe I18n do
  context "html safety" do
    it "should not return a SafeBuffer if no SafeBuffers are interpolated" do
      ...
      expect(translation.html_safe?).to be_falsey
      expect(translation).to eql("I want 2 widgets from Acme Co.")
    end
    ...
    it "should return a SafeBuffer if a SafeBuffer is interpolated" do
      ...
      expect(translation.html_safe?).to be_truthy
      expect(translation).to eql("I want <input> widgets")
    end
    ...
    it "should html_escape the translation if a SafeBuffer is interpolated" do
      ...
      expect(translation.html_safe?).to be_truthy
      expect(translation).to eql("If you create an &lt;input&gt; tag, you will see <input>")
    end
    ...
    it "should apply :wrapper" do
      ...
      expect(translation.html_safe?).to be_truthy
      expect(translation).to eq %{This is <em class="super-important">important</em>}
    end
    ...
    it "should apply :wrappers" do
      ...
      expect(translation.html_safe?).to be_truthy
      expect(translation).to eq %{User <span class="firstname">User</span> <span class="lastname">Amp&amp;Name</span>}
    end

    it "should apply all duplicate wrappers" do
      translation = I18n.t(:foo, 'From *here* to *there*', :wrapper => '<span>\1</span>')
      expect(translation.html_safe?).to be_truthy
      expect(translation).to eq %{From <span>here</span> to <span>there</span>}
    end
    ...
    it "should handle similar looking wrappers" do
      translation = I18n.t(:foo, 'From *here* to **there** and ***everywhere***',
                           :wrapper => { '*' => '<span class="1">\1</span>',
                               ...
                               '***' => '<span class="3">\1</span>' })
      expect(translation.html_safe?).to be_truthy
      expect(translation).to eq %{From <span class="1">here</span> to <span class="2">there</span> and <span class="3">everywhere</span>}
    end
  end

  context "pluralization" do
    it "should accept hashes for the default value" do
      expect(I18n.t(:foo, {:one => "1 thing", :other => "%{count} things"}, :count => 1)).
        to eq "1 thing"
      expect(I18n.t(:foo, {:one => "1 thing", :other => "%{count} things"}, :count => 2)).
        to eq "2 things"
    end

    it "should pluralize single-word default values" do
      expect(I18n.t(:foo, "thing", :count => 1)).
        to eq "1 thing"
      expect(I18n.t(:foo, "thing", :count => 2)).
        to eq "2 things"
    end

    it "should not pluralize multi-word default values" do
      expect(I18n.t(:foo, "thing count: %{count}", :count => 1)).
        to eq "thing count: 1"
      expect(I18n.t(:foo, "thing count: %{count}", :count => 2)).
        to eq "thing count: 2"
    end
  end
