...
SwaggerModel.prototype.setReferencedModels = function(allModels) {
  ...
  for (var i = 0; i < this.properties.length; i++) {
    ...
    if (allModels[type] != null) results.push((property.refModel = allModels[type]))
...
var SwaggerModelProperty = function(name, obj) {
  ...
  this.dataTypeWithRef =
    this.refDataType != null ? this.dataType + '[' + this.refDataType + ']' : this.dataType
...
var SwaggerOperation = function(
  ...
  if (errors.length > 0)
    console.error(
      'Errors while initializing SwaggerOperation:',
      errors,
      nickname,
      path,
      method,
  ...
  if (typeof this.type !== 'undefined' && this.type === 'void') this.type = null
  else {
...
var SwaggerRequest = function(
  ...
  this.type =
    type || errors.push('SwaggerRequest type is required (get/post/put/delete/patch/options).')
  this.url = url || errors.push('SwaggerRequest url is required.')
  this.params = params
  this.opts = opts
  this.successCallback =
    successCallback || errors.push('SwaggerRequest successCallback is required.')
  ...
  for (var i = 0; i < params.length; i++) {
    var param = params[i]
    if (param.paramType === 'form') formParams.push(param)
    else if (param.paramType === 'file') fileParams.push(param)
  }
  ...
  if (body && (this.type === 'POST' || this.type === 'PUT' || this.type === 'PATCH')) {
    ...
    if (formParams.length > 0) {
      if (fileParams.length > 0) requestContentType = 'multipart/form-data'
      else requestContentType = 'application/x-www-form-urlencoded'
  ...
  if (requestContentType && requestContentType.indexOf('application/x-www-form-urlencoded') === 0) {
    ...
    var splitStrip = function(str) {
      return _.map(str.split(','), function(v) {
        return v.trim()
      })
    }
  ...
  if (requestContentType && body) myHeaders['Content-Type'] = requestContentType
...
ApiKeyAuthorization.prototype.apply = function(obj, authorizations) {
  if (this.type === 'query') {
    if (obj.url.indexOf('?') > 0) obj.url = obj.url + '&' + this.name + '=' + this.value
