...
module DocRedirect
  module UrlMapper
    OLD_BASE_URL = "https://canvas.cd.instructure.com/doc/api"
    NEW_BASE_URL = "https://developerdocs.instructure.com/services/canvas"

    class << self
      # Generate URL mappings from TOC content and save to output file
      # @param toc_content [String] Content of toc.md (rendered ERB)
      # @param output_file [String] Path to save url_mappings.json
      def generate(toc_content, output_file)
        mappings = parse_toc(toc_content)
      ...
      # Parse the toc.md content and extract mappings
