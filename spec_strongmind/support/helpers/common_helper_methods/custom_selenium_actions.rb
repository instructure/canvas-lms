...
module CustomSeleniumActions
  ...
  def fln(link_text, scope = nil)
    stale_element_protection do
      (scope || page).find :link, link_text
    end
  end
  ...
  def ff(selector, scope = nil)
    reloadable_collection do
      (scope || page.driver).all(:css, selector)
    end
  end
  ...
  def find_with_jquery(selector, scope = nil)
    page.driver.execute_script("return $(arguments[0], arguments[1] && $(arguments[1]))[0];", selector, scope)
  end

  def find_all_with_jquery(selector, scope = nil)
    page.driver.execute_script("return $(arguments[0], arguments[1] && $(arguments[1])).toArray();", selector, scope)
  end
  ...
  def in_frame(id)
    ...
    page.driver.switch_to.frame(id)
    begin
      yield
    ensure
      page.driver.switch_to.window saved_window_handle
    end
  end
  ...
  def get_value(selector)
    page.driver.execute_script("return $(#{selector.inspect}).val()")
  end
  ...
  def dialog_for(node)
    node.find(:xpath, "ancestor-or-self::div[contains(@class, 'ui-dialog')]") rescue false
  end
  ...
  def find_css_in_string(string_of_html, css_selector)
    page.driver.execute_script("return $('<div />').append('#{string_of_html}').find('#{css_selector}')")
  end
  ...
  def select_all_in_tiny(tiny_controlling_element)
    ...
    page.driver.execute_script(src)
  end
  ...
  def type_in_tiny(tiny_controlling_element, text, clear: false)
    ...
    keep_trying_until do
      page.driver.execute_script("return $(#{selector}).siblings('.mce-tinymce').length > 0;")
    end

    iframe_id = page.driver.execute_script("return $(#{selector}).siblings('.mce-tinymce').find('iframe')[0];")['id']
    ...
    if text.length > 100 || text.lines.size > 1
      ...
      page.driver.execute_script("return $(#{selector}).val(#{html.inspect})")
      switch_editor_views(tiny_controlling_element)
    else
      in_frame iframe_id do
        tinymce_element = f("body")
  ...
  def hover_and_click(element_jquery_finder)
    if fj(element_jquery_finder).present?
      page.driver.execute_script(%{$(#{element_jquery_finder.to_s.to_json}).trigger('mouseenter').click()})
    end
  end
  ...
  def force_click(element_jquery_finder)
    ...
    page.driver.execute_script(%{$(#{element_jquery_finder.to_s.to_json}).click()})
  end
  ...
  def set_value(input, value)
    case input.tag_name
      when 'select'
        input.find(:css, "option[value='#{value}']").click
      when 'input'
  ...
  def datepicker_next(day_text = '15')
    datepicker = f('#ui-datepicker-div')
    datepicker.find(:css, '.ui-datepicker-next').click
    fj("#ui-datepicker-div a:contains(#{day_text})").click
    datepicker
  end
  ...
  def replace_content(el, value, options = {})
    ...
    when :chrome
      page.driver.execute_script("arguments[0].select()", el)
      keys = [:backspace]
    when :safari
      el.clear()
      keys = []
  ...
  def submit_form(form)
    ...
    button = form.is_a?(Selenium::WebDriver::Element) ? form.find(:css, submit_button_css) : f("#{form} #{submit_button_css}")
    button.click
  end
  ...
  def submit_dialog(dialog, submit_button_css = ".ui-dialog-buttonpane .button_type_submit")
    ...
    dialog.all(:css, submit_button_css).last.click
  end
  ...
  def drag_with_js(selector, x, y)
    load_simulate_js
    page.driver.execute_script "$('#{selector}').simulate('drag', { dx: #{x}, dy: #{y} })"
  end
  ...
  def error_displayed?
    ...
    page.driver.execute_script("return $('.error_text:visible').filter(function(){ return $(this).offset().left >= 0 }).length > 0")
  end
  ...
  def replace_value(selector, value)
    page.driver.execute_script("$('#{selector}').val(#{value})")
  end

  def current_active_element
    page.driver.switch_to.active_element
  end
  ...
  def scroll_to(element)
    ...
    page.driver.execute_script('window.scrollTo(0, ' + element_location + ');')
  end
  ...
  def scroll_into_view(selector)
    page.driver.execute_script("$(#{selector.to_json})[0].scrollIntoView()")
  end
  ...
  def scroll_element(selector, target)
    page.driver.execute_script("$(#{selector.to_json}).scrollTo(#{target.to_json})")
  end
