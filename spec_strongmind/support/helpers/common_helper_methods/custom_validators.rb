...
module CustomValidators
  def validate_breadcrumb_link(link_element, breadcrumb_text)
    ...
    expect(execute_script("return INST.errorCount;")).to eq 0
  end

  def check_domready
    dom_is_ready = execute_script "return window.seleniumDOMIsReady"
    requirejs_resources_loaded = execute_script "return !requirejs.s.contexts._.defQueue.length"
  ...
  def check_element_has_focus(element)
    active_element = execute_script('return document.activeElement')
  ...
  def assert_error_box(selector)
    box = execute_script <<-JS, selector
  ...
  def wait_for_new_page_load(accept_alert = false)
    execute_script("window.INST = window.INST || {}; INST.still_on_old_page = true;")
    yield
    wait_for(method: :wait_for_new_page_load) do
      begin
        execute_script("return window.INST && INST.still_on_old_page !== true;")
