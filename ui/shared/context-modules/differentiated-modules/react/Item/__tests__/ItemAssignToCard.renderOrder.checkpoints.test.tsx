...
describe('ItemAssignToCard - Render Order Checkpoints', () => {
  beforeAll(() => {
    server.listen()
  ...
  it('renders checkpoint date inputs in correct order', () => {
    const {getByLabelText, getAllByLabelText} = renderComponent({
      isCheckpointed: true,
    })

    // Verify all checkpoint date inputs render
    ...
    // The order should be: Reply to Topic, Required Replies, Available From, Until
    ...
    expect(dateInputContainers).toHaveLength(4)
    ...
    expect(testIds[0]).toBe('reply_to_topic_due_at_input')
    expect(testIds[1]).toBe('required_replies_due_at_input')
    expect(testIds[2]).toBe('unlock_at_input')
    expect(testIds[3]).toBe('lock_at_input')
  })
})
