...
describe('PeerReviewAvailableToDateTimeInput', () => {
  const defaultProps = {
  ...
  const renderComponent = (overrides = {}) =>
    render(<PeerReviewAvailableToDateTimeInput {...defaultProps} {...overrides} />)
  ...
  it('renders with null date value', () => {
    renderComponent({peerReviewAvailableToDate: null})
    const dateInput = screen.getByLabelText('Until')
    expect(dateInput).toBeInTheDocument()
    expect(dateInput).toHaveValue('')
  })

  it('renders with a date value', () => {
    const dateValue = '2024-11-15T23:59:00Z'
    renderComponent({peerReviewAvailableToDate: dateValue})
    const dateInput = screen.getByLabelText('Until')
    expect(dateInput).toBeInTheDocument()
  })
  ...
  it('calls setPeerReviewAvailableToDate with null when clear button is clicked', async () => {
    const setPeerReviewAvailableToDate = jest.fn()
    renderComponent({
      peerReviewAvailableToDate: '2024-11-15T23:59:00Z',
  ...
  it('calls handlePeerReviewAvailableToDateChange when date is changed', async () => {
    ...
    await waitFor(() => {
      expect(handlePeerReviewAvailableToDateChange).toHaveBeenCalled()
    })
  })
  ...
  it('calls handleBlur when input loses focus', () => {
    ...
    expect(handleBlur).toHaveBeenCalledWith('peer_review_available_to')
  ...
  describe('validation messages', () => {
    ...
    it('displays error message when validation error exists', () => {
      renderComponent({
        validationErrors: {peer_review_available_to: 'Date must be in the future'},
        showMessages: true,
      })
    ...
    it('displays invalid date message when field is unparsed', () => {
      const unparsedFieldKeys = new Set(['peer_review_available_to'])
      renderComponent({
        unparsedFieldKeys,
        showMessages: true,
      })
  ...
  it('renders with showMessages prop', () => {
    const {rerender, container} = renderComponent({
      showMessages: false,
      validationErrors: {peer_review_available_to: 'Some error'},
    })
