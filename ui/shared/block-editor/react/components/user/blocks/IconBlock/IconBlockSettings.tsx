/*
 * Copyright (C) 2024 - present Instructure, Inc.
 *
 * This file is part of Canvas.
 *
 * Canvas is free software: you can redistribute it and/or modify it under
 * the terms of the GNU Affero General Public License as published by the Free
 * Software Foundation, version 3 of the License.
 *
 * Canvas is distributed in the hope that it will be useful, but WITHOUT ANY
 * WARRANTY; without even the implied warranty of MERCHANTABILITY or FITNESS FOR
 * A PARTICULAR PURPOSE. See the GNU Affero General Public License for more
 * details.
 *
 * You should have received a copy of the GNU Affero General Public License along
 * with this program. If not, see <http://www.gnu.org/licenses/>.
 */

import React, {useCallback} from 'react'
import {useNode} from '@craftjs/core'
import {Text} from '@instructure/ui-text'
import {View} from '@instructure/ui-view'

import {iconMap} from '../../../../assets/icons'
import {type IconBlockProps} from './IconBlock'

const IconBlockSettings = () => {
  const {
    actions: {setProp},
    props,
  } = useNode(node => ({
    props: node.data.props,
  }))

  const handleSelectIcon = useCallback(
    (iconName: string) => {
      setProp((prps: IconBlockProps) => (prps.iconName = iconName))
    },
    [setProp]
  )

  const handleKey = useCallback(
    (event: React.KeyboardEvent, iconName: string) => {
      if (event.key === 'Enter') {
        handleSelectIcon(iconName)
      }
    },
    [handleSelectIcon]
  )

  const selectedStyle = {borderColor: 'var(--ic-brand-primary)'}
  const iconButtonStyle = {
    margin: '4px',
    padding: '2px',
    display: 'inline-block',
    cursor: 'pointer',
    borderWidth: '1px',
    borderStyle: 'solid',
    borderColor: 'transparent',
    borderRadius: '4px',
  }

  const renderNoIcon = () => {
    const isSelected = !props.iconName
    let style = {...iconButtonStyle}
    if (isSelected) style = {...style, ...selectedStyle}

    return (
      <div
        role="button"
        style={style}
        tabIndex={0}
        onClick={() => handleSelectIcon('')}
        onKeyDown={(event: React.KeyboardEvent) => handleKey(event, '')}
      >
        <Text size="small">No Icon</Text>
      </div>
    )
  }

  return (
    <View as="div" margin="small" background="secondary" borderWidth="small" maxWidth="17rem">
      {Object.keys(iconMap).map(iconName => {
        const Icon = iconMap[iconName]
        const isSelected = iconName === props.iconName
        let style = {...iconButtonStyle}
        if (isSelected) {
          style = {...style, ...selectedStyle}
        }
        return (
          <div
            key={iconName}
            role="button"
            style={style}
            tabIndex={0}
            onClick={() => handleSelectIcon(iconName)}
            onKeyDown={(event: React.KeyboardEvent) => handleKey(event, iconName)}
          >
            <Icon size="small" />
          </div>
        )
      })}
      {renderNoIcon()}
    </View>
  )
}

export {IconBlockSettings}
