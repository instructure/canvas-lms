...
describe('jquery.toJSON', () => {
  ...
  test('serializes to a JSON string correctly', () => {
    ...
    expect(JSON.stringify(expected)).toBe(JSON.stringify(form))
  })

  test(`returns null if element with datetime_field enabled class has undefined for $.data( 'date' )`, () => {
    form.prepend($datepickerEl())
    expect(form.toJSON().date).toBeNull()
  })
  ...
  test('returns date object for form element with datetime_field_enabled', () => {
    ...
    $dateEl.val(date)
    expect(form.toJSON().date).toBe(date)
  })
})
