...
_.init = function (cmd, html_template, text_template, replacedFragment) {
  const self = this // minifier optimization
...
_.placeCursor = function (cursor) {
  ...
  cursor.appendTo(
    this.foldChildren(this.firstChild, (prev, child) => (prev.isEmpty() ? prev : child))
  )
}
_.isEmpty = function () {
...
_.each = function (fn) {
  for (let el = this.prev.next || this.parent.firstChild; el !== this.next; el = el.next)
...
function createRoot(jQ, root, textbox, editable, include_toolbar) {
  const contents = jQ.contents().detach()
  ...
  root.jQ = jQ.data(jQueryDataKey, {
    ...
    revert() {
      jQ.empty()
        .unbind('.mathquill')
        .removeClass(
          'mathquill-rendered-math mathquill-editable mathquill-textbox mathquill-editor'
        )
        .append(contents)
    },
  })

  const cursor = (root.cursor = new Cursor(root))

  root.renderLatex(contents.text())

  // textarea stuff
  const textareaSpan = (root.textarea = $('<span class="textarea"><textarea></textarea></span>')),
    textarea = textareaSpan.children()
  if (include_toolbar) addToolbar(root, jQ)

  let textareaSelectionTimeout
  ...
  function setTextareaSelection() {
    ...
    const latex = cursor.selection ? '$' + cursor.selection.latex() + '$' : ''
    textarea.val(latex)
    if (latex) {
      if (textarea[0].select) textarea[0].select()
      else if (document.selection) {
        const range = textarea[0].createTextRange()
  ...
  function paste() {
    ...
    let latex = textarea.val()
    if (latex.slice(0, 1) === '$' && latex.slice(-1) === '$') latex = latex.slice(1, -1)
    else if (!latex.match(/^\\/))
      // make it text if it doesn't look like latex
      latex = '\\text{' + latex + '}'
  ...
  function textInput() {
    ...
    } else if (cursor.selection || textareaSelectionTimeout !== undefined) setTextareaSelection()
  }
}
...
function addToolbar(root, jQ) {
  ...
  const html_template_overrides = {
    binomial:
      '<span style="font-size: 0.48em"><span class="paren" style="font-size: 2.087912087912088em; ">(</span><span class="array"><span><var>n</var></span><span><var>m</var></span></span><span class="paren" style="font-size: 2.087912087912088em; ">)</span></span>',
    ...
    nthroot:
      '<span style="font-size: 0.7em"><sup class="nthroot"><var>n</var></sup><span class="block"><span class="sqrt-prefix">&radic;</span><span class="sqrt-stem">&nbsp;</span></span></span>',
    supscript: '<sup style="font-size: 0.6em">sup</sup>',
    subscript: '<sub style="font-size: 0.6em; line-height: 3.5;">sub</sub>',
    vector:
      '<span class="array" style="vertical-align: middle; font-size: 0.4em; line-height: 0.9em"><span>1</span><span>2</span><span>3</span></span>',
  }
  ...
  $.each(button_tabs, (index, tab) => {
    tabs.push(
      '<li><a href="#' +
        htmlEscape(tab.name) +
        '_tab" role="tab" tabindex="-1" aria-controls="' +
        htmlEscape(tab.name) +
        '_tab">' +
        '  <span class="mathquill-rendered-math">' +
        htmlEscape(tab.example) +
        '</span>' +
        htmlEscape(tab.name) +
        '</a></li>'
    )
    ...
    $.each(tab.button_groups, (index, group) => {
      $.each(group, (index, cmd) => {
        const obj = new LatexCmds[cmd](undefined, cmd)
        buttons.push(
          '<li><a class="mathquill-rendered-math" href="#" title="' +
            htmlEscape(cmd.match(/^[a-z]+$/i) ? '\\' + cmd : cmd) +
            '">' +
            $.raw(
              html_template_overrides[cmd]
                ? html_template_overrides[cmd]
                : '<span style="line-height: 1.5em">' +
                    $.raw(obj.html_template.join('')) +
                    '</span>'
            ) +
            '</a></li>'
        )
    ...
    panes.push(
      '<div class="mathquill-tab-pane" id="' +
        htmlEscape(tab.name) +
        '_tab" role="tabpanel"><ul>' +
        $.raw(buttons.join('')) +
        '</ul></div>'
    )
  })
  root.toolbar = $('#mathquill-view .mathquill-toolbar').html(
    '<ul class="mathquill-tab-bar" role="tablist">' +
      $.raw(tabs.join('')) +
      '</ul><div class="mathquill-toolbar-panes">' +
      $.raw(panes.join('')) +
      '</div>'
  )
  $('#mathquill-view .mathquill-tab-bar li a')
    .click(function (e) {
      e.preventDefault()
      $('#mathquill-view .mathquill-tab-bar li')
        .removeClass('mathquill-tab-selected')
        .find('a')
        .attr('tabindex', '-1')
    ...
    .keydown(function (e) {
      let direction, listIndex, $tabLinks
      switch (e.keyCode) {
        case 37:
          direction = 'l'
          break
  ...
  $('#mathquill-view a.mathquill-rendered-math')
    .mousedown(e => {
      e.stopPropagation()
    })
    .click(function () {
...
_.latex = function () {
  return MathBlock.prototype.latex.call(this).replace(/(\\[a-z]+) (?![a-z])/gi, '$1')
}
_.text = function () {
...
_.keydown = function (e) {
  ...
  switch ((e.originalEvent && e.originalEvent.keyIdentifier) || e.which) {
    ...
    case 'Backspace':
    case 'U+0008':
      if (e.ctrlKey) while (this.cursor.prev || this.cursor.selection) this.cursor.backspace()
    ...
    case 'Del':
    case 'U+007F':
      if (e.ctrlKey) while (this.cursor.next || this.cursor.selection) this.cursor.deleteForward()
...
function RootMathCommand(cursor) {
  ...
  this.firstChild.textInput = function (ch) {
    ...
    if (ch !== '$' || cursor.parent !== this) cursor.write(ch)
...
_.renderLatex = function (latex) {
  ...
  for (let i = 0; i < latex.length; i += 1) {
    ...
    if (chunk[0] === '$') {
      ...
      const root = new RootMathCommand(cursor)
      cursor.insertNew(root)
      root.firstChild.renderLatex(chunk)
      cursor.show().insertAfter(root)
    } else {
      for (let j = 0; j < chunk.length; j += 1) this.cursor.insertNew(new VanillaSymbol(chunk[j]))
    }
  }
}
...
if (transformPropName) {
  ...
  scale = function (jQ, x, y) {
    ...
    const intervalId = setInterval(calculateMarginRight)
    $(window).load(() => {
...
_.latex = function () {
  const latex = this.firstChild.latex()
...
_.respace = function () {
  if (
    this.prev.cmd === '\\int ' ||
    (this.prev instanceof SupSub &&
      this.prev.cmd != this.cmd &&
      this.prev.prev &&
      this.prev.prev.cmd === '\\int ')
  ) {
    if (!this.limit) {
      this.limit = true
  ...
  if (this.respaced) {
    const fontSize = +this.jQ.css('fontSize').slice(0, -2),
...
_.placeCursor = function (cursor) {
  ...
  if (this.firstChild.isEmpty()) {
    ...
    while (
      prev &&
      !(prev instanceof BinaryOperator || prev instanceof TextBlock || prev instanceof BigSymbol) // lookbehind for operator
    )
      prev = prev.prev
    ...
    if (prev !== this.prev) {
      ...
      const newBlock = new MathFragment(this.parent, prev, this).blockify()
      newBlock.jQ = this.firstChild.jQ
        .empty()
        .removeClass('empty')
        .append(newBlock.jQ)
        .data(jQueryDataKey, {block: newBlock})
...
_.initBlocks = function (replacedFragment) {
  ...
  const block = (this.blockjQ = this.firstChild.jQ)
...
_.redraw = function () {
  const height = this.blockjQ.outerHeight() / +this.blockjQ.css('fontSize').slice(0, -2)
...
LatexCmds.lbrack =
  LatexCmds.lbracket =
  CharCmds['['] =
    proto(Paren, function (replacedFragment) {
      Paren.call(this, '[', ']', replacedFragment)
    })
...
LatexCmds.rbrack =
  LatexCmds.rbracket =
  CharCmds[']'] =
    proto(CloseParen, function (replacedFragment) {
      CloseParen.call(this, '[', ']', replacedFragment)
    })
...
_.placeCursor = function (cursor) {
  ...
  if (this.replacedText)
    for (let i = 0; i < this.replacedText.length; i += 1) this.write(this.replacedText.charAt(i))
}
_.write = function (ch) {
...
_.keydown = function (e) {
  ...
  if (
    ...
    ((e.which === 8 && !this.cursor.prev) || (e.which === 46 && !this.cursor.next))
  ) {
...
_.textInput = function (ch) {
  ...
  if (ch !== '$') this.write(ch)
    ...
    // split apart
    const next = new TextBlock(new MathFragment(this.firstChild, this.cursor.prev))
...
_.renderCommand = function () {
  ...
  if (latex) this.cursor.insertCmd(latex, this.replacedFragment)
    ...
    const cmd = new VanillaSymbol('\\backslash ', '\\')
...
function Binomial(replacedFragment) {
  ...
  this.bracketjQs = $('<span class="paren">(</span>')
    .prependTo(this.jQ)
...
_.keydown = function (e) {
  const currentBlock = this.cursor.parent
...
LatexCmds.alpha =
  ...
  LatexCmds.psi =
  LatexCmds.omega =
    proto(Symbol, function (replacedFragment, latex) {
...
_.respace = function () {
  if (!this.prev) {
    ...
    this.next &&
    !(this.next instanceof BinaryOperator)
  ) {
...
LatexCmds.notin =
  ...
  LatexCmds.oplus =
  LatexCmds.otimes =
    proto(BinaryOperator, function (replacedFragment, latex) {
...
_.respace = function () {
  this.jQ[0].className =
    this.next instanceof SupSub || this.next instanceof Bracket ? '' : 'non-italicized-function'
}
...
;(function () {
  const trig = ['sin', 'cos', 'tan', 'sec', 'cosec', 'csc', 'cotan', 'cot']
  for (const i in trig) {
    LatexCmds[trig[i]] =
      LatexCmds[trig[i] + 'h'] =
      LatexCmds['a' + trig[i]] =
...
function Cursor(root) {
  ...
  const jQ = (this.jQ = this._jQ = $('<span class="cursor">&zwj;</span>'))
...
_.redraw = function () {
  for (let ancestor = this.parent; ancestor; ancestor = ancestor.parent)
    if (ancestor.redraw) ancestor.redraw()
}
...
_.seek = function (target, pageX, pageY) {
  const cursor = this.clearSelection()
  if (target.hasClass('empty')) {
    cursor.prependTo(target.data(jQueryDataKey).block)
    return cursor
  }

  let data = target.data(jQueryDataKey)
  if (data) {
  ...
  // if no MathQuill data, try parent, if still no, forget it
    ...
    if (!data) data = {block: cursor.root}
  }
  ...
  let dist = cursor.jQ.offset().left - pageX,
    prevDist
  do {
    cursor.moveLeft()
    prevDist = dist
...
_.resolveNonItalicizedFunctions = function () {
  let node = this.prev
  let raw = node ? node.latex().replace(/ $/, '') : null
  let count = 0
  const functions = {
    ln: 1,
    lg: 1,
  ...
  let latex = ''
  while (node && raw) {
    const single_char = raw.match(/^[a-z]$/)
    if (
      single_char ||
      (latex &&
        raw[0] == '\\' &&
...
_.writeLatex = function (latex, noMoveCursor) {
  ...
  latex = (latex && latex.match(/\\text\{([^}]|\\\})*\}|\\:|\\[a-z]*|[^\s]/gi)) || 0
  ;(function writeLatexBlock(cursor) {
    while (latex.length) {
      let token = latex.shift() // pop first item
      ...
      if (token.slice(0, 6) === '\\text{') {
        ...
        if (cursor.prev)
          // was a close-paren, so break recursion
          return
        // was an open-paren, hack to put the following latex
        else latex.unshift('{') // in the ParenBlock in the math DOM
...
_.write = function (ch) {
  const ret = this.show().insertCh(ch)
...
_.selectFrom = function (anticursor) {
  ...
  loopThroughAncestors: while (true) {
    ...
    for (let otherI = anticursor; otherI !== otherA.parent.parent; otherI = otherI.parent.parent)
  ...
  if (left.next !== right) {
    for (let next = left; next; next = next.next) {
      if (next === right.prev) {
        leftRight = true
        break
      }
...
_.detach = function () {
  const block = MathFragment.prototype.blockify.call(this)
  ...
  switch (cmd) {
    case 'redraw':
      this.find(':not(:has(:first))').each(function () {
        const data = $(this).data(jQueryDataKey)
