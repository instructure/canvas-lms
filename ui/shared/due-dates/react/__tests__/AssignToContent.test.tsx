...
describe('AssignToContent', () => {
  ...
  const props = {
    ...
    overrides: (assignmentcollection as any).models.map(
      (model: any) => model.toJSON().assignment_override,
    ),
    defaultSectionId: 0,
  }
  ...
  beforeAll(() => {
    ...
    window.ENV.COURSE_ID = COURSE_ID as any
  ...
  describe('important dates', () => {
    ...
    it('disables the importantDates check when no due dates are set', () => {
      const override = [(assignmentcollection as any).models[0]]
