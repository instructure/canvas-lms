...
const setup = ({
  ...
  return render(
    <AssignmentDueDate
      // @ts-expect-error TS2739 (typescriptify)
...
describe('AssignmentDueDate', () => {
  ...
  describe('AssignmentDueDate callbacks', () => {
    it('updates dueDate on change and triggers callback', async () => {
      ...
      // Validate callback date without assuming a specific timezone offset.
      // CI environments can run in different timezones, which changes the ISO time component.
      const [{dueDate}] = onAssignedInfoChange.mock.calls.at(-1) ?? []
      expect(dueDate).toBeTruthy()
      expect(DateTime.toLocaleString(dueDate, locale, timeZone, 'LL')).toBe('April 4, 2018')
    })
  })
})
