...
describe('SignIn', () => {
  ...
  describe('login behavior', () => {
    ...
    it('displays a login error alert when invalid credentials are submitted', async () => {
      ...
      await userEvent.click(loginButton)
      await waitFor(() => {
        const loginError = screen.getByText(/Please verify your Email or password and try again/i)
  ...
  describe('ui State', () => {
    ...
    it('removes the error alert when the user starts typing in the username field', async () => {
      ...
      const loginButton = screen.getByTestId('login-button')
      await userEvent.click(loginButton)
      const usernameError = await screen.findByText(/Please enter your Email/i)
  ...
  describe('form validation', () => {
    it('shows validation error when the username contains only whitespace', async () => {
      ...
      await userEvent.type(usernameInput, '{backspace}{backspace}user@example.com')
      await userEvent.click(loginButton)
    ...
    it('shows validation error when the password contains only whitespace after fixing the username', async () => {
      ...
      await userEvent.type(passwordInput, '{backspace}{backspace}password123')
      await userEvent.click(loginButton)
    ...
    it('shows validation messages when fields are left blank', async () => {
      const loginButton = screen.getByTestId('login-button')
      await userEvent.click(loginButton)
      const emailError = await screen.findByText(/Please enter your Email/i)
    ...
    it('validates fields sequentially: email first, then password', async () => {
      const loginButton = screen.getByTestId('login-button')
      await userEvent.click(loginButton)
