...
const PersonalInfo = ({portfolio}: PersonalInfoProps) => {
  const {heroImageUrl, blurb, city, state, phone, email, about} = portfolio
  ...
  return (
    ...
    >
      <Flex as="div" direction="column" gap="large" margin="medium 0">
        <div
          style={{position: 'relative', height: '184px', background: '#C7CDD1', overflow: 'hidden'}}
        >
          {heroImageUrl ? <Img src={heroImageUrl} alt="Cover image" /> : null}
          <div style={{position: 'absolute', right: '12px', bottom: '12px'}}>
            <Button renderIcon={IconEditLine}>Edit cover image</Button>
          </div>
