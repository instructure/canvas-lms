...
export default function LicenseHelpModal() {
  const licenseSelect = useRef<HTMLSelectElement | null>(null)
  const licenseIcon = useRef<HTMLElement | null>(null)
  ...
  useEffect(() => {
    licenseIcon.current = document.querySelector('.license_help_link')
    if (licenseIcon.current) {
      licenseIcon.current.addEventListener('click', event => {
