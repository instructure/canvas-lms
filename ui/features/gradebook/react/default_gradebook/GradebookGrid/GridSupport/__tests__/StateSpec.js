...
QUnit.module('GradebookGrid GridSupport State', suiteHooks => {
  let grid
  let gridSupport
  ...
  QUnit.module('with item metadata', () => {
    ...
    test('applies the row "cssClass" to each row', () => {
      ...
      strictEqual(
        $rows[0],
        document.querySelector('.slick-row.row_A'),
        '"row_A" is applied to the first row'
      )
      strictEqual(
        $rows[1],
        document.querySelector('.slick-row.row_B'),
        '"row_B" is applied to the second row'
      )
    })
  })
})
