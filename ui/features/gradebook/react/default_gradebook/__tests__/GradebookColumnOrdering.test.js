...
describe('Gradebook Grid Column Ordering', () => {
  ...
  describe('when sorting by module position', () => {
    it('sorts assignment columns by module position', () => {
      createGradebookAndAddData()
      // Remove assignment_2303 from the grid columns
      const columns = gradebook.gradebookGrid.grid.getColumns()
      ...
      const expectedOrder = [
        ...
        'assignment_2302', // Second Module
        'assignment_2301', // Final Module
      ]
      const sortedColumns = gradebook.gradebookGrid.grid.getColumns()
      const actualOrder = sortedColumns.map(column => column.id)
      expect(actualOrder).toEqual(expectedOrder)
    })
  })
})
