...
import {copyCourseMutation} from '../copyCourseMutation'
...
describe('copyCourseMutation', () => {
  ...
  it('should create a new course and copy content', async () => {
    ...
    const contentMigrationResponse = {ok: true}

    mockDoFetchApi.mockResolvedValueOnce({json: courseCreationResult})
    mockDoFetchApi.mockResolvedValueOnce({response: contentMigrationResponse})
    mockConvertFormDataToMigrationCreateRequest.mockReturnValueOnce(mockReturnValue)

    const result = await copyCourseMutation({accountId, courseId, formData})
  ...
  it('should throw an error if course creation fails', async () => {
    ...
    await expect(copyCourseMutation({accountId, courseId, formData})).rejects.toThrow(
  ...
  it('should throw an error if content migration fails', async () => {
    const courseCreationResult = {id: '4'}
    const contentMigrationResponse = {ok: false}
