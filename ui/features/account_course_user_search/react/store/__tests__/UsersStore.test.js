...
describe('UsersStore', () => {
  describe('normalizeParams', () => {
    test('omits search_term argument if not given', () => {
      const {search_term} = UsersStore.normalizeParams({})
      expect(search_term).toBeUndefined()
    })

    test('omits search_term argument if blank', () => {
      const {search_term} = UsersStore.normalizeParams({search_term: ''})
      expect(search_term).toBeUndefined()
    })

    test('strips whitespace from search_term argument', () => {
      const {search_term} = UsersStore.normalizeParams({search_term: ' foo  '})
