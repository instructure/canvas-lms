...
describe AgliveComUtil do
  ...
  it "should schedule a class" do
    ...
    XML
    Net::HTTP.any_instance.expects(:request).with { |req| req.is_a?(Net::HTTP::Post) && Rack::Utils.parse_nested_query(req.body).should == expected_request }.returns(response)
    aglive.schedule_class(class_params).should == expected_response
  end
  ...
  it "should add an attendee (student) to a class" do
    ...
    Net::HTTP.any_instance.expects(:request).with do |req|
      ...
      req.is_a?(Net::HTTP::Post) && Hash.from_xml(body.delete('attendee_list')).should == expected_attendee_xml && body.should == expected_request
    end.returns(response)
    aglive.add_attendee_to_session("test_id", 1234, "test student")["attendee_url"].should == "http://example.com/attendee_url"
  end
end
