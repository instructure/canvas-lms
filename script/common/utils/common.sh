...
# Chekck if docker-compose is aliased to docker compose
if docker-compose version &> /dev/null; then
  DOCKER_COMMAND=${DOCKER_COMMAND:-"docker-compose"}
elif docker compose version &> /dev/null; then
  DOCKER_COMMAND=${DOCKER_COMMAND:-"docker compose"}
else
  echo "docker-compose or docker compose command not found. Please install Docker Compose."
  exit 1
fi
...
function rebuild_docker_images {
  start_spinner "Rebuilding docker images..."
    if [[ "${OS:-}" == 'Linux' && -z "${CANVAS_SKIP_DOCKER_USERMOD:-}" ]]; then
      _canvas_lms_track_with_log $DOCKER_COMMAND build --pull --build-arg USER_ID=$(id -u)
    else
