...
await Promise.all(packageJsons.map(async (path) => {
  const contents = Object.assign({ path: relative(canvasDir, path) }, JSON.parse(await readFile(path)));
...
if(argv.includes('-i')) {
  if(invalid.length > 0) {
    ...
    if(argv.includes('-g')) {
      invalid.forEach((contents) => {
        execFileSync('gergich', ['comment', JSON.stringify({
          path: contents.path,
