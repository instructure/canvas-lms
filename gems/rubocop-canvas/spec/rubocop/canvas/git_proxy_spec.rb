  ...
  describe GitProxy do
    describe ".dirty?" do
      it "is true for any change list" do
        ...
        expect(described_class.new.dirty?(changes)).to be(true)
      end

      it "is false for an empty list" do
        expect(described_class.new.dirty?("")).to be(false)
      end

      it "doesnt count whitespace as changes" do
        expect(described_class.new.dirty?(" \n ")).to be(false)
      end
    end

    describe ".head_sha" do
      subject(:sha){ described_class.new.head_sha }
