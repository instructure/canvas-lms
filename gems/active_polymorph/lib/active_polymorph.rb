...
RAILS2 = ActiveRecord::VERSION::STRING < '3'

module ActiveRecord
  if RAILS2
    class Associations::HasManyAssociation
      def construct_sql_with_polymorphic_names
        construct_sql_without_polymorphic_names
        if @reflection.options[:as]
          unless @reflection.options[:finder_sql]
            @finder_sql =
              "#{@reflection.quoted_table_name}.#{@reflection.options[:as]}_id = #{owner_quoted_id} AND " +
              "#{@reflection.quoted_table_name}.#{@reflection.options[:as]}_type IN (#{@owner.class.polymorphic_names.map{|a| @owner.class.quote_value(a) }.join(",")})"
            @finder_sql << " AND (#{conditions})" if conditions
          end
          @counter_sql = @finder_sql unless @reflection.options[:counter_sql]
        end
      end
      alias_method_chain :construct_sql, :polymorphic_names
    end
  else
    class Relation
