...
module AccountReports
  class OutcomeReports
    ...
    def student_assignment_outcome_map
      if Account.site_admin.feature_enabled?(:improved_outcomes_report_generation)
        write_outcomes_report(
          self.class.student_assignment_outcome_headers,
          improved_student_assignment_outcome_map_scope,
          { post_process_record: method(:post_process_student_assignment_outcome_map_scope_record) }
        )
      else
    ...
    def post_process_student_assignment_outcome_map_scope_record(record_hash, cache)
    ...
    def improved_student_assignment_outcome_map_scope
      parameters = {
        account_id: account.id,
        root_account_id: root_account.id
      }
    ...
    def write_outcomes_report(headers, canvas_scope, config_options = {})
      improved_outcomes_report_generation_enabled = Account.site_admin.feature_enabled?(:improved_outcomes_report_generation)
      ...
      write_report headers, enable_i18n_features do |csv|
        ...
        if improved_outcomes_report_generation_enabled
