...
const getInformationRow = (dueAt, points) => {
  ...
  if (points) {
    toDisplay.push(
      <ListItem key="points">{I18n.t('%{numPoints} points', {numPoints: points})}</ListItem>
    )
  }
  toDisplay.push(
    <ListItem key="date">
...
export default function ToDoItem(props) {
  ...
  const titleComponent = props.href ? <Link href={props.href}>{title}</Link> : <Text>{title}</Text>
  ...
  return (
    <div className="ToDoSidebarItem">
      ...
      <div className="ToDoSidebarItem__Info">
        ...
        <List variant="pipe">{getInformationRow(props.dueAt, props.points)}</List>
      </div>
      <div className="ToDoSidebarItem__Close">
        <Button variant="icon" size="small" onClick={handleClick}>
