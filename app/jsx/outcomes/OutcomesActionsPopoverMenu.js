  ...
  render: function() {
    if (this.state.menuItems.length > 0) {
      var renderedMenuItems = this.state.menuItems.map(function(menuItem) {
        return (
          <MenuItem key={menuItem.key} onSelect={menuItem.onSelect}>
            {menuItem.name}
          </MenuItem>
        )
      })
      ...
      return (
        <span>
          <PopoverMenu
            ...
            trigger={
              <Button>
                <ScreenReaderContent>{I18n.t('Additional outcomes options')}</ScreenReaderContent>&hellip;
              </Button>
            }
          >
            {renderedMenuItems}
          ...
          <AddToCourseModal
            ...
            onReady={this._handleModalReady}
          />
        </span>
      )
    }
