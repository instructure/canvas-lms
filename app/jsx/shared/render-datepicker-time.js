...
function renderDatepickerTime($input) {
  ...
  var label = <label htmlFor="ui-datepicker-time-hour">{STRINGS.timeLabel}</label>
  ...
  if (tz.useMeridian()) {
    ...
    meridianSelect = (
      <select className="ui-datepicker-time-ampm un-bootrstrapify" title={STRINGS.selectTitle}>
        ...
        <option value={STRINGS.AM} selected={data.ampm == 'am'} key="am">
          {STRINGS.AM}
        </option>
        <option value={STRINGS.PM} selected={data.ampm == 'pm'} key="pm">
          {STRINGS.PM}
        </option>
      </select>
    )
