...
function __sortMarkColumn(state, action, column, provisionalGradeIndex) {
  ...
  var previouslySortedAsDescending =
    state.sort.column === column && state.sort.direction === DESCENDING
...
studentHandlers[ModerationActions.SELECT_MARK] = (state, action) => {
  ...
  var studentIndex = newState.students.indexOf(studentObj)
  if (studentIndex > -1) {
    newState.students[studentIndex].selected_provisional_grade_id =
      action.payload.selectedProvisionalId
  }
  return newState
}
