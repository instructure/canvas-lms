...
var ModerationActions = {
  ...
  selectProvisionalGrade(selectedProvisionalId, ajaxLib) {
    return (dispatch, getState) => {
      var endpoint =
        getState().urls.provisional_grades_base_url + '/' + selectedProvisionalId + '/select'
      ajaxLib = ajaxLib || axios
      ajaxLib
        .put(endpoint)
        .then(response => {
          dispatch(
            this.selectedProvisionalGrade(
              response.data.student_id,
              response.data.selected_provisional_grade_id
            )
          )
        })
        .catch(response => {
          dispatch(
            this.selectingProvisionalGradesFailed(
              I18n.t('An error occurred selecting provisional grades')
            )
          )
        })
    }
  ...
  publishGrades(ajaxLib) {
    return (dispatch, getState) => {
      ...
      ajaxLib
        ...
        .then(response => {
          dispatch(
            this.publishedGrades(I18n.t('Success! Grades were published to the grade book.'))
          )
        })
        .catch(response => {
          const errorMessages = {
          ...
          let message =
            errorMessages[response.status] || I18n.t('An error occurred publishing grades.')
  ...
  addStudentToModerationSet(ajaxLib) {
    return (dispatch, getState) => {
      ...
      ajaxLib
        .post(endpoint, {
          student_ids: _.reduce(
            getState().studentList.students,
            (ids, student) => {
