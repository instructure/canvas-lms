...
export default class AnnouncementList extends React.Component {
  ...
  renderAnnouncement() {
    return this.props.announcements.map(c => (
      <tr key={c.id}>
        <td>
          <ToggleDetails
            summary={TextHelper.truncateText(c.title, {max: 100})}
            className="AnnouncementList__message"
          >
            <div
              className="AnnouncementList__message-content"
    ...
    if (this.props.announcements.length) {
      return (
        <Table caption={I18n.t('Recent Announcements')} striped="rows">
          <tbody>{this.renderAnnouncement()}</tbody>
        </Table>
      )
    }
    return null
  ...
  render() {
    return <div className="AnnouncementList">{this.renderTable()}</div>
  }
}
