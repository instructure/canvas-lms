...
var GradingStandard = React.createClass({
  ...
  insertGradingStandardRow: function(index) {
    var newEditingStandard = update(this.state.editingStandard, {
  ...
  rowDataIsValid: function() {
    ...
    var rowValues = _.map(this.state.editingStandard.data, function(dataRow) {
      return String(dataRow[1]).trim()
    })
    var sanitizedRowValues = _.chain(rowValues)
      .compact()
      .uniq()
      .value()
    var inputsAreUniqueAndNonEmpty = sanitizedRowValues.length === rowValues.length
    var valuesDoNotOverlap = !_.any(
  ...
  rowNamesAreValid: function() {
    var rowNames = _.map(this.state.editingStandard.data, function(dataRow) {
      return dataRow[0].trim()
    })
    var sanitizedRowNames = _.chain(rowNames)
      .compact()
      .uniq()
      .value()
      ...
      return (
        <div>
          <button
            ref="editButton"
            className={
              'Button Button--icon-action edit_grading_standard_button ' +
  ...
  renderDisabledIcons: function() {
    if (
      ...
      var url =
        '/' + splitAssetString(this.props.standard.context_code).join('/') + '/grading_standards'
