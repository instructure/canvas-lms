...
describe('Account Course User Search CoursesList Sorting', () => {
  ...
  test('displays courses in the right order', () => {
    ...
    expect(renderedList.nodes[0].props.name).toBe('A')
    expect(renderedList.nodes[1].props.name).toBe('Ba')
    expect(renderedList.nodes[2].props.name).toBe('Bb')
    expect(renderedList.nodes[3].props.name).toBe('C')
    expect(renderedList.nodes[4].props.name).toBe('De')
    expect(renderedList.nodes[5].props.name).toBe('Dz')

    expect(renderedList.nodes[0].props.id).toBe('1')
    expect(renderedList.nodes[1].props.id).toBe('2')
    expect(renderedList.nodes[2].props.id).toBe('3')
    expect(renderedList.nodes[3].props.id).toBe('4')
    expect(renderedList.nodes[4].props.id).toBe('5')
    expect(renderedList.nodes[5].props.id).toBe('6')
  });

  test('displays Terms in right order', () => {
    const renderedList = shallow(<CoursesList {...coursesProps} />).find(CoursesListRow)

    expect(renderedList.nodes[0].props.term.name).toBe('A Term')
    expect(renderedList.nodes[1].props.term.name).toBe('Ba Term')
    expect(renderedList.nodes[2].props.term.name).toBe('Bb Term')
    expect(renderedList.nodes[3].props.term.name).toBe('C Term')
    expect(renderedList.nodes[4].props.term.name).toBe('De Term')
    expect(renderedList.nodes[5].props.term.name).toBe('Dz Term')
  })
})
