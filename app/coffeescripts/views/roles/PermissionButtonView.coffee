define [
  ...
  class PermissionButtonView extends Backbone.View
    ...
    events:
      ...
      "keydown input[type=radio]": "handleKeydown"
    ...
    # @api private
    updateRole: (event) ->
    updateRole: (event, options = {}) ->
      event.preventDefault()
      id = options?.id || $(event.target).attr('id')
      switch id
        when "button-#{@cid}-0"
    ...
    handleClick: (event) ->
      event.preventDefault()
      @updateRole(event, {id: $(event.target).closest('label').attr('for')})
      @setPreviewIcons()
      @closeMenu()
      @saveModel()
    ...
    closeMenu: ->
      ...
      # a11y: Make sure focus goes back to the button used to open the menu
      @$el.children('.btn-group').find('a').focus()
