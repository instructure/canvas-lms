...
module Auditors::ActiveRecord
  class AccountUserRecord < ActiveRecord::Base
    ...
    self.table_name = "auditor_account_user_records"

    belongs_to :root_account, class_name: "Account", inverse_of: :auditor_account_user_records
    belongs_to :account_user, class_name: "::AccountUser", inverse_of: :auditor_records
    belongs_to :performing_user,
               class_name: "User",
               optional: true,
               inverse_of: :auditor_performing_user_account_user_records
    ...
    class << self
      ...
      def ar_attributes_from_event_stream(record)
        record.attributes.except("id").tap do |attrs_hash|
          ...
          attrs_hash["account_user_id"] = Shard.relative_id_for(record.account_user_id, Shard.current, Shard.current)
        end
      end
    end
  end
