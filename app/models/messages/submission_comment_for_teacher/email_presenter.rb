...
module Messages::SubmissionCommentForTeacher
  class EmailPresenter < Presenter
    def subject
      if anonymous?
        I18n.t(
          "Anonymous Submission Comment: Student (%{id}), %{assignment_title}, %{course_name}",
    ...
    def can_reply?
      !anonymous? && IncomingMailProcessor::MailboxAccount.reply_to_enabled
    end

    def show_avatars?
      !anonymous? && message.avatar_enabled?
    end
  end
end
