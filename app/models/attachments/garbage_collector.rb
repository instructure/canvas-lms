...
class Attachments::GarbageCollector
  class ByContextType
    attr_reader :context_type, :older_than
    def initialize(context_type:, older_than:)
    ...
    def delete_content
      to_delete_scope.where(root_attachment_id: nil).find_ids_in_batches(batch_size: 500) do |ids_batch|
        ...
        Attachment.where(id: ids_batch).each do |att|
          next if older_than && same_type_children_max_created_at &&
            same_type_children_max_created_at >= older_than

          if non_type_children[att.id].present?
            att.make_childless(non_type_children[att.id].first)
  ...
  class FolderContextType < ByContextType
    def initialize
      super(context_type: 'Folder', older_than: nil)
    end
  end
  ...
  class ContentExportContextType < ByContextType
    def initialize(older_than:)
      super(context_type: 'ContentExport', older_than: older_than)
    end
