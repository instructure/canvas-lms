...
class Quizzes::QuizEligibility
  ...
  def restricted_by_date?
    ...
    # closed| open    | closed   | false          | true
    # closed| open    | closed   | true           | true
    # closed| closed  | open     | false          | false
    ...
    return false if section_is_ongoing?
    return false unless term_has_ended? || course_has_ended?
    true
  end
  ...
  def assignment_overrides
    AssignmentOverride.where(quiz_id: quiz.id).map(&:set)
  end
  ...
  def course_has_ended?
    return true if course_has_soft_concluded?
    return false unless course.restrict_enrollments_to_course_dates
    course.end_at && course.end_at <= Time.zone.now
  end

  def course_has_soft_concluded?
    course.restrict_enrollments_to_course_dates && course.soft_concluded?
  end
