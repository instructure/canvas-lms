...
class ContextModulesController < ApplicationController
  ...
  module ModuleIndexHelper
    ...
    def load_modules
      ...
      @collapsed_modules = ContextModuleProgression.for_user(@current_user).for_modules(@modules).pluck(:context_module_id, :workflow_state).select{|cm_id, workflow_state| workflow_state != "started" }.map(&:first)
