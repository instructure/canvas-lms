...
class FilesController < ApplicationController
  ...
  def api_show
    ...
    if @attachment.context_type == "Course" && (@attachment.locked || @attachment.hidden || @attachment.locked_for?(@current_user)) && !@attachment.context.grants_right?(@current_user, session, :view_unpublished_items)
      render json: { errors: [{ message: "The specified resource does not exist." }] }, status: :not_found
      return
    end
    params[:include] = Array(params[:include])
