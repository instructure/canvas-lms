...
module K5Mode
  ...
  def set_k5_mode(require_k5_theme: false)
    ...
    if @context.try(:elementary_enabled?) || (require_k5_theme && k5_user?)
      css_bundle :k5_theme
      css_bundle :k5_font if !@current_user&.prefers_dyslexic_font? && ((@context.is_a?(Course) && !@context.account.use_classic_font_in_k5?) || (!@context.is_a?(Course) && !use_classic_font?))
