...
const isHandlebarsAssignmentNode = function (node) {
  const result = node.body.contains(childNode => childNode.constructor.name === 'Call' &&
      childNode.variable.base.value === 'Handlebars' &&
      childNode.variable.contains(registerNode => registerNode.constructor.name === 'Access' &&
          registerNode.name.value === 'registerHelper'));
  return result;
}

const findHelperNodes = function (helpersAST) {
...
const buildQueryStringElements = function (helperNodes) {
  const queryElements = [];
