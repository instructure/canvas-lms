class UniquifyWikis < ActiveRecord::Migration
  ...
  def self.up
    ...
    rows = connection.select_rows("SELECT context_type, context_id FROM wiki_namespaces WHERE namespace='default' GROUP BY context_type, context_id HAVING COUNT(*) > 1")
