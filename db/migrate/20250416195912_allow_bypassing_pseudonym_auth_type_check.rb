...
class AllowBypassingPseudonymAuthTypeCheck < ActiveRecord::Migration[7.1]
  tag :postdeploy
  disable_ddl_transaction!

  def up
    # This migration was written _after_ NormalizePseudonyms, but needs to run _before_ it.
    # But if that migration has already successfully run, we shouldn't run this one
    versions = ActiveRecord::Base.schema_migration.versions
    return if versions.include?("20250417195912")
    ...
    connection.execute(<<~SQL) # rubocop:disable Rails/SquishedSQLHeredocs
      ...
      BEGIN
          ...
          IF OLD.auth_type IS DISTINCT FROM NEW.auth_type AND current_setting('inst.backfilling_pseudonyms_auth_type', true) IS NULL THEN
