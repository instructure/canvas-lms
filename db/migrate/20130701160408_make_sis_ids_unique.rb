class MakeSisIdsUnique < ActiveRecord::Migration
  ...
  def self.up
    add_index :accounts, [:sis_source_id, :root_account_id], conditions: "sis_source_id IS NOT NULL", unique: true, algorithm: :concurrently
    add_index :accounts, :root_account_id, algorithm: :concurrently
    add_index :courses, [:sis_source_id, :root_account_id], conditions: "sis_source_id IS NOT NULL", unique: true, algorithm: :concurrently
    add_index :course_sections, [:sis_source_id, :root_account_id], conditions: "sis_source_id IS NOT NULL", unique: true, algorithm: :concurrently
    add_index :enrollment_terms, [:sis_source_id, :root_account_id], conditions: "sis_source_id IS NOT NULL", unique: true, algorithm: :concurrently
    ...
    add_index :pseudonyms, [:sis_user_id, :account_id], conditions: "sis_user_id IS NOT NULL", unique: true, algorithm: :concurrently
    add_index :pseudonyms, :account_id, algorithm: :concurrently
    add_index :groups, [:sis_source_id, :root_account_id], conditions: "sis_source_id IS NOT NULL", unique: true, algorithm: :concurrently
