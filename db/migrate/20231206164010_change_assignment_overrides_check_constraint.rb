...
class ChangeAssignmentOverridesCheckConstraint < ActiveRecord::Migration[7.0]
  tag :predeploy
  ...
  def up
    ...
    add_check_constraint(:assignment_overrides,
                         "workflow_state='deleted' OR assignment_id IS NOT NULL OR quiz_id IS NOT NULL OR context_module_id IS NOT NULL OR wiki_page_id IS NOT NULL OR discussion_topic_id IS NOT NULL OR attachment_id IS NOT NULL",
                         name: :require_association,
                         if_not_exists: true,
                         validate: false)
    validate_constraint(:assignment_overrides, :require_association)
  end

  def down
    remove_check_constraint(:assignment_overrides, name: "require_association", if_exists: true)
    add_check_constraint(:assignment_overrides,
