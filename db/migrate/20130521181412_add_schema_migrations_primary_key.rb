class AddSchemaMigrationsPrimaryKey < ActiveRecord::Migration
  ...
  def self.up
    case connection.adapter_name
      ...
      when 'MySQL', 'Mysql2'
        execute("ALTER TABLE #{connection.quote_table_name('schema_migrations')} ADD PRIMARY KEY (version)")
  ...
  def self.down
    case connection.adapter_name
      ...
      when 'MySQL', 'Mysql2'
        execute("ALTER TABLE #{connection.quote_table_name('schema_migrations')} DROP PRIMARY KEY")
        add_index :schema_migrations, :version, :unique => true, :name => 'unique_schema_migrations'
    end
  end
end
