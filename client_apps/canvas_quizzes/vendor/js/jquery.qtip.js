...
;(function(window, document, undefined) {
  ...
  ;(function(factory) {
    ...
    PROTOTYPE.reposition = function(event, effect) {
      ...
      position.left +=
        adjust.x + (my.x === RIGHT ? -tooltipWidth : my.x === CENTER ? -tooltipWidth / 2 : 0)
      position.top +=
        adjust.y + (my.y === BOTTOM ? -tooltipHeight : my.y === CENTER ? -tooltipHeight / 2 : 0)
    ...
    C.string = function() {
      ...
      return x === y
        ? x
        : this.precedance === Y || (this.forceY && y !== 'center') ? y + ' ' + x : x + ' ' + y
    }
    ...
    function hideMethod(event) {
      ...
      if (
        (this !== relatedTarget[0] && (this.options.position.target === 'mouse' && ontoTooltip)) ||
    ...
    function init(elem, id, opts) {
      var obj,
        ...
        metadata5 =
          opts.metadata.type === 'html5' && metadata ? metadata[opts.metadata.name] : NULL,
      ...
      reposition: function(event, api, pos, viewport) {
        ...
        if (this.corner.fixed !== TRUE) {
          ...
          if (
            ...
            (cache.cornerTop !== adjust.top || cache.cornerLeft !== adjust.left)
          ) {
    ...
    PLUGINS.viewport = function(
      ...
      function calculate(
        ...
        var initialPos = position[side1],
          ...
          isShift = type === SHIFT,
          myLength =
            mySide === side1 ? elemLength : mySide === side2 ? -elemLength : -elemLength / 2,
          atLength =
            atSide === side1 ? targetLength : atSide === side2 ? -targetLength : -targetLength / 2,
          sideOffset =
          ...
          offset =
            myLength -
            (my.precedance === side || mySide === my[otherSide] ? atLength : 0) -
      ...
      adjusted = {
        left:
          methodX !== 'none'
            ? calculate(X, Y, methodX, adjust.x, LEFT, RIGHT, WIDTH, targetWidth, elemWidth)
            : 0,
        top:
          methodY !== 'none'
            ? calculate(Y, X, methodY, adjust.y, TOP, BOTTOM, HEIGHT, targetHeight, elemHeight)
            : 0
      }
