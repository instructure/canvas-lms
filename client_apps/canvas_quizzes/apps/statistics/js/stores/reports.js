  ...
  return new Store(
    'quizReports',
    {
      ...
      populate: function(payload, options) {
        ...
        if (options.track) {
          quizReports.where({isGenerating: true}).forEach(this.trackReportGeneration.bind(this))
        }
      ...
      actions: {
        generate: function(reportType, onChange, onError) {
          ...
          quizReports.generate(reportType).then(
            function(quizReport) {
        ...
        regenerate: function(reportId, onChange, onError) {
          ...
          quizReports.generate(quizReport.get('reportType')).then(
            function retrackGeneration(quizReport) {
